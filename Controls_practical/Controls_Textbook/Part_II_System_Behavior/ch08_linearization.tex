...
Joseph-Louis Lagrange (1736--1813) systematized this approach in his \textit{M\'{e}canique Analytique} (1788), developing a general theory of small oscillations for systems with multiple degrees of freedom. His method involved:
\begin{enumerate}
\item Finding the equilibrium configuration
\item Expanding the potential and kinetic energies to second order
\item Deriving linearized equations of motion
\item Solving for the ``normal modes'' of oscillation
\end{enumerate}
...
\begin{itemize}
\item Prediction of natural frequencies (phugoid, short period modes)
\item Design of control surfaces for adequate stability margins
\item Analysis of pilot-in-the-loop handling qualities
\item Development of autopilot systems
\end{itemize}

The success of linearized aircraft dynamics established the paradigm that dominates control engineering: \textit{design around an operating point}.

\subsection{Why Linearize? The Practical Argument}

Nonlinear differential equations are \textbf{hard}. Specifically:

\begin{enumerate}
\item \textbf{No superposition:} For linear systems, if $x_1(t)$ and $x_2(t)$ are solutions, so is $\alpha x_1(t) + \beta x_2(t)$. This allows us to analyze complex inputs by decomposing them into simpler components (Fourier analysis, convolution). Nonlinear systems do not have this property.

\item \textbf{No general solution methods:} Linear differential equations with constant coefficients can always be solved using eigenvalue methods. Most nonlinear equations have no closed-form solutions.

\item \textbf{No transfer functions:} The Laplace transform converts linear ODEs to algebraic equations, enabling frequency-domain analysis. This transformation is not applicable to nonlinear systems.

\item \textbf{Limited stability theory:} For linear systems, stability depends only on eigenvalue locations. Nonlinear systems can exhibit limit cycles, chaos, and other behaviors that linear theory cannot predict.
\end{enumerate}

Linear systems, by contrast, are mathematically tractable:
\begin{itemize}
\item Complete solution via eigenvalue decomposition
\item Transfer function representation and frequency response
\item Root locus and Bode plot design methods
\item State feedback and observer design via pole placement
\item Optimal control via LQR and Kalman filtering
\end{itemize}
...
\begin{itemize}
\item Aerodynamic forces as functions of angle of attack, sideslip, Mach number
\item Engine thrust characteristics across the flight envelope
\item Atmospheric variations with altitude
\item Actuator dynamics and rate limits
\end{itemize}
...
\begin{itemize}
\item Arrhenius temperature dependence of reaction rates: $k = A e^{-E_a/RT}$
\item Nonlinear heat transfer (especially with phase changes)
\item Concentration-dependent reaction kinetics
\item Multiple steady states and potential for runaway
\end{itemize}
...
\begin{itemize}
\item Loop gain analysis for stability
\item Compensator design using Bode plots
\item Input/output impedance calculations
\item Prediction of transient response
\end{itemize}
...
\begin{itemize}
\item Resolved-motion rate control
\item Cartesian impedance control
\item Force/position hybrid control
\end{itemize}
...
\begin{itemize}
\item $\theta = 0$ (hanging down) --- stable equilibrium
\item $\theta = \pi$ (inverted) --- unstable equilibrium
\end{itemize}
...
\begin{enumerate}
\item \textbf{Magnitude of perturbations:} The higher-order terms in the Taylor expansion scale as $(\delta x)^2$, $(\delta x)^3$, etc. A rule of thumb is that linearization is valid when $|\delta x / x_0| < 0.1$ (10\% deviation), but this depends on the specific nonlinearity.

\item \textbf{Smoothness of the nonlinearity:} Functions with high curvature (large second derivatives) have smaller validity regions than functions with low curvature.

\item \textbf{Time scale of interest:} Errors in the linearized model accumulate over time. For short-duration transients, linearization may be adequate even for moderate perturbations; for long-term predictions, small errors can grow substantially.

\item \textbf{Purpose of the model:} If the model is used for stability analysis (determining whether perturbations grow or decay), linearization is appropriate. If quantitative predictions of transient response are needed, the validity region is more restrictive.
\end{enumerate}
...
\begin{itemize}
\item 3D-printed pendulum arm (suggested length: 20--30 cm)
\item Low-friction pivot (ball bearing or needle bearing)
\item Rotary encoder or potentiometer for angle measurement
\item Arduino or similar microcontroller
\item Optional: Servo motor for controlled release
\item Frame or mounting structure
\end{itemize}
...
\begin{itemize}
\item Uniform cross-section for predictable moment of inertia
\item Hole for pivot bearing at one end
\item Small mass concentration at the opposite end (or attachment point for additional mass)
\item Recommended material: PLA or PETG, 20--30\% infill
\end{itemize}
...
\begin{enumerate}
\item Mount the pendulum and verify free swinging with minimal friction.
\item Release the pendulum from a small angle ($\theta_0 = 10°$).
\item Record the time for 10 complete oscillations; calculate the period $T_{10}$.
\item Repeat for initial angles of $20°, 30°, 45°, 60°, 75°, 90°$.
\item For each measurement, take at least 3 trials and compute the average.
\end{enumerate}
...
\begin{lstlisting}[language=C++, caption={Arduino Period Measurement}]
// Pseudocode for period measurement
volatile long lastCrossing = 0;
...
void zeroCrossing() {
  long now = micros();
  period = 2.0 * (now - lastCrossing) / 1e6; // seconds
  lastCrossing = now;
}

void loop() {
  // Print period when stable
...
\begin{enumerate}
\item Configure the Arduino to log angle vs. time at high sample rate (e.g., 1 kHz).
\item Release from $\theta_0 = 15°$ and record several periods.
\item Repeat for $\theta_0 = 45°$ and $\theta_0 = 75°$.
\item Export data for analysis in MATLAB, Python, or spreadsheet software.
\end{enumerate}
...
\begin{enumerate}
\item Implement a simple proportional controller: $\tau = -K_p \theta$
\item Test regulation to $\theta = 0$ from small initial angles (should work well).
\item Test from large initial angles (may exhibit oscillations or instability).
\item Observe how controller performance degrades outside the linear region.
\end{enumerate}
...
\begin{enumerate}
\item Constant (linear prediction)
\item Theoretical nonlinear period (from elliptic integral or series expansion)
\end{enumerate}
...
\begin{itemize}
\item The waveform ``flattens'' near the peaks (spends more time at large angles)
\item The period is longer than predicted
\item Higher harmonics appear in the Fourier spectrum
\end{itemize}
...
\begin{enumerate}
\item At what amplitude does the measured period deviate from the linear prediction by more than 5\%? How does this compare to the theoretical threshold?

\item If friction is present, how does it affect the comparison between linear and nonlinear models?

\item For the closed-loop control experiment: Why does the controller work better for small angles? What modifications might extend its operating range?

\item How would you design an experiment to linearize about the inverted equilibrium ($\theta = \pi$)?
\end{enumerate}
...
\subsection{Example 1: Pendulum with Torque Input}
...
\subsection{Example 2: Tank Level System}
...
\begin{lstlisting}[language=Python, caption={Nonlinear Pendulum Simulation}]
import numpy as np
...
# Linear solution
theta_lin = theta0 * np.cos(omega_n * t)
...
# Plot comparison
plt.plot(t, np.degrees(theta_lin), '--', label='Linear')
plt.plot(t, np.degrees(theta_nonlin), '-', label='Nonlinear')
plt.xlabel('Time (s)')
...
# Linear period
T_lin = 2*np.pi/omega_n  # 2.006 s

# Nonlinear period (from simulation)
# Find first two zero-crossings
crossings = np.where(np.diff(np.sign(theta_nonlin)))[0]
T_nonlin = 2 * (t[crossings[1]] - t[crossings[0]])

print(f"Linear period: {T_lin:.4f} s")
print(f"Nonlinear period: {T_nonlin:.4f} s")
print(f"Error: {100*(T_nonlin-T_lin)/T_lin:.2f}%")
\end{lstlisting}

\textbf{Results:}
\begin{itemize}
\item Linear period: $T_{\text{lin}} = 2.006$ s
\item Nonlinear period: $T_{\text{nonlin}} \approx 2.040$ s
\item Error: $\approx 1.7\%$
\end{itemize}
...
\begin{enumerate}
\item \textbf{Perturbation variables:} The linearized model describes deviations from the operating point, not absolute values. When implementing a controller, remember that:
\begin{align}
\delta x &= x - x_0 \quad (\text{measured state minus setpoint}) \\
\delta u &= u - u_0 \quad (\text{control input minus nominal})
\end{align}

\item \textbf{Bias terms:} In implementation, the actual control signal is:
\begin{equation}
u = u_0 + \delta u = u_0 + K(\delta x)
\end{equation}
The term $u_0$ provides the steady-state input needed to maintain the operating point; $\delta u$ is the corrective action from the controller.

\item \textbf{Gain scheduling:} If the system operates over a range of conditions (e.g., aircraft at different altitudes/speeds), multiple linearizations are performed and controller gains are ``scheduled'' based on the current operating condition.

\item \textbf{Operating envelope:} The set of operating points for which a single linearized controller provides acceptable performance defines the operating envelope. Beyond this envelope, the controller may exhibit degraded performance or instability.
\end{enumerate}
...
\begin{enumerate}
\item \textbf{Taylor series} provides the mathematical foundation: any smooth function can be approximated by a polynomial, and locally, by a linear function.

\item \textbf{Equilibrium points} are the natural operating points for linearization, where $f(x_0, u_0) = 0$.

\item \textbf{Jacobian matrices} $A$ and $B$ capture the local sensitivity of the dynamics to state and input perturbations.

\item \textbf{Validity} of the linear approximation depends on the magnitude of perturbations and the curvature of the nonlinearity. For most control applications, perturbations within 10--20\% of the operating point yield acceptable accuracy.

\item \textbf{Eigenvalue analysis} of the linearized system determines local stability properties.
\end{enumerate}

The linearization approach has proven remarkably successful across engineering disciplines because:
\begin{itemize}
\item Well-designed control systems keep perturbations small
\item Linear tools (transfer functions, Bode plots, state feedback) are powerful and well-understood
\item Gain scheduling extends linear methods to cover wide operating ranges
\end{itemize}
...
\begin{enumerate}
\item Linearize the system $\dot{x} = -x^3 + u$ about the equilibrium with $u_0 = 0$. Find the transfer function and determine stability.

\item A satellite attitude control system has dynamics:
\begin{equation}
...
\item For the tank system of Example 2, how does the time constant change if the operating level is doubled from 4 m to 8 m?

\item Linearize the Lotka-Volterra predator-prey equations:
\begin{align}
...
\item An inverted pendulum on a cart has the approximate linearized dynamics (for small angles):
\begin{equation}
...
\item For the pendulum experiment: if the bearing friction contributes a damping torque $-b\dot{\theta}$, how does this affect (a) the equilibrium analysis, (b) the linearized model, and (c) the validity of the linear approximation?

\item (Computer Exercise) Implement the pendulum simulation of Example 5. Extend it to initial angles of $15°, 45°, 60°, 75°, 90°$. Create a plot showing the period error (compared to linear prediction) as a function of initial amplitude. At what amplitude does the error exceed 5\%?
\end{enumerate}
...
\begin{itemize}
\item Khalil, H.K., \textit{Nonlinear Systems}, 3rd ed. Prentice Hall, 2002. Chapter 4 provides rigorous treatment of linearization and stability.

\item Slotine, J.J.E. and Li, W., \textit{Applied Nonlinear Control}. Prentice Hall, 1991. Excellent coverage of when linearization fails and what to do about it.

\item Strogatz, S.H., \textit{Nonlinear Dynamics and Chaos}, 2nd ed. Westview Press, 2015. Accessible introduction to nonlinear systems with beautiful examples.

\item For historical context: Diacu, F. and Holmes, P., \textit{Celestial Encounters: The Origins of Chaos and Stability}. Princeton University Press, 1996.
\end{itemize}
