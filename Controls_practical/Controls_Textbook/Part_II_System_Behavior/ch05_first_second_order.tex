...
This realization forced seismologists to develop rigorous theories of second-order system response. If the seismograph had natural frequency $\omega_n$ and damping ratio $\zeta$, then:

\begin{itemize}
    \item For ground oscillations at frequencies much \textit{lower} than $\omega_n$, the seismograph mass would move with the ground (displacement measurement).
    \item For ground oscillations at frequencies much \textit{higher} than $\omega_n$, the mass would remain stationary while the frame moved (acceleration measurement).
    \item For frequencies \textit{near} $\omega_n$, resonance would amplify or distort the signal.
\end{itemize}
...
\begin{itemize}
    \item Mass $m$: the sprung mass (vehicle body)
    \item Spring constant $k$: provided by coil springs, leaf springs, or torsion bars
    \item Damping coefficient $c$: provided by hydraulic shock absorbers
\end{itemize}
...
\begin{itemize}
    \item Stable: all poles in the left half-plane ($\real(s) < 0$)
    \item Marginally stable: poles on the imaginary axis ($\real(s) = 0$)
    \item Unstable: any pole in the right half-plane ($\real(s) > 0$)
\end{itemize}
...
\begin{itemize}
    \item \textbf{First-order systems} arise when there is \textit{one} energy storage element (capacitor, spring, thermal mass) and \textit{one} dissipative element (resistor, damper, thermal resistance).
    \item \textbf{Second-order systems} arise when there are \textit{two} energy storage elements that can exchange energy (inductor-capacitor, mass-spring, kinetic-potential energy).
\end{itemize}
\end{keyconceptbox}
...
\begin{itemize}
    \item \textbf{Time constant significance:} At $t = \tau$, the output reaches $63.2\%$ of its final value:
    \begin{equation}
    y(\tau) = K(1 - e^{-1}) = K(1 - 0.368) = 0.632K
    \end{equation}

    \item \textbf{Settling time} (2\% criterion): Time to reach and stay within 2\% of final value:
    \begin{equation}
    t_s \approx 4\tau \quad \text{(since } e^{-4} \approx 0.018 \approx 2\%)
    \end{equation}

    \item \textbf{Rise time} (10\% to 90\%):
    \begin{equation}
    t_r = \tau(\ln 0.9 - \ln 0.1) = \tau \ln 9 \approx 2.2\tau
    \end{equation}

    \item \textbf{Initial slope:} The tangent at $t=0$ has slope $K/\tau$ and intersects $y=K$ at $t=\tau$.
\end{itemize}
\end{keyconceptbox}
...
where:
\begin{itemize}
    \item $\omega_n > 0$ is the \textbf{natural frequency} (rad/s)
    \item $\zeta \geq 0$ is the \textbf{damping ratio} (dimensionless)
\end{itemize}
\end{definition}
...
\begin{enumerate}
    \item \textbf{Underdamped} ($0 < \zeta < 1$): Complex conjugate poles
    \begin{equation}
    ...
    \item \textbf{Critically damped} ($\zeta = 1$): Repeated real pole
    \begin{equation}
    s_{1,2} = -\omega_n \quad \text{(double pole)}
    \end{equation}
...
\begin{enumerate}
    \item \textbf{Horizontal distance from imaginary axis} ($\sigma = \zeta\omega_n$): Determines the exponential decay rate. Larger $\sigma$ means faster settling.

    \item \textbf{Vertical distance from real axis} ($\omega_d = \omega_n\sqrt{1-\zeta^2}$): Determines the oscillation frequency. Larger $\omega_d$ means faster oscillation.

    \item \textbf{Distance from origin} ($\omega_n = \sqrt{\sigma^2 + \omega_d^2}$): The natural frequency.

    \item \textbf{Angle from negative real axis} ($\theta = \arccos\zeta$): Directly related to damping ratio.
\end{enumerate}
...
\begin{enumerate}
    \item Construct a physical mass-spring-damper system with variable damping
    \item Measure step response using digital instrumentation
    \item Extract $\zeta$ and $\omega_n$ from experimental data
    \item Correlate physical damping changes with observed response characteristics
\end{enumerate}
\end{experimentbox}
...
\begin{itemize}
    \item Vertical guide rail (aluminum or 3D-printed): 300mm length
    \item Mass carrier: 3D-printed or machined block, approximately 100g base mass
    \item Additional masses: washers, nuts, or calibrated weights (50g, 100g, 200g)
    \item Linear bearings or low-friction sleeve for vertical motion
    \item Mounting base: stable platform to prevent vibration
\end{itemize}
...
\begin{itemize}
    \item Use consistent rubber bands (size \#64 or similar)
    \item Parallel bands increase $k$; series arrangement decreases $k$
    \item Alternative: extension springs from hardware store with known spring constant
\end{itemize}
...
\textbf{Option A: Water Dashpot}
\begin{itemize}
    \item Cylinder partially filled with water
    \item Plunger attached to moving mass with orifices of various sizes
    \item Vary damping by changing orifice size or water viscosity (add glycerin)
\end{itemize}

\textbf{Option B: Eddy Current Damper}
\begin{itemize}
    \item Attach aluminum or copper plate to moving mass
    \item Position permanent magnets near plate path
    \item Vary damping by adjusting magnet proximity
    \item Provides smooth, velocity-proportional damping
\end{itemize}

\textbf{Option C: Friction Pad (Coulomb Damping Approximation)}
\begin{itemize}
    \item Felt or foam pad pressed against guide rail
    \item Adjust normal force with thumbscrew
    \item Note: provides approximately velocity-independent friction (not ideal viscous damping)
\end{itemize}
...
\begin{itemize}
    \item Ultrasonic sensor (HC-SR04): range 2cm--400cm, resolution $\sim$3mm
    \item Time-of-flight sensor (VL53L0X): range 3cm--200cm, resolution $\sim$1mm
    \item Infrared distance sensor (Sharp GP2Y0A21): range 10cm--80cm
\end{itemize}

\textbf{Data Acquisition:}
\begin{itemize}
    \item Arduino Uno or similar microcontroller
    \item Sampling rate: minimum 50 Hz, preferably 100+ Hz
    \item Data logging to serial monitor or SD card
\end{itemize}
...
\begin{enumerate}
    \item \textbf{Measure mass:} Weigh the total moving mass $m$ including carrier, sensor mount, and any added weights.

    \item \textbf{Measure spring constant:} With damping mechanism removed, displace mass and measure displacement. Apply several known forces and calculate $k$ from the slope of force vs.\ displacement.

    \item \textbf{Predict natural frequency:} Calculate $\omega_n = \sqrt{k/m}$ and $f_n = \omega_n/(2\pi)$.

    \item \textbf{Verify undamped frequency:} With minimal damping, apply step input and measure oscillation period $T$. Compare $f_{measured} = 1/T$ with prediction.
\end{enumerate}

\subsubsection{Step Response Measurement}

\begin{enumerate}
    \item Position the mass at a displaced equilibrium (e.g., compress spring by 2--3 cm).

    \item Start data acquisition.

    \item Release mass smoothly (avoid imparting initial velocity).

    \item Record position vs.\ time for at least 5 natural periods or until settling.

    \item Repeat measurement 3--5 times for statistical confidence.
\end{enumerate}
...
\begin{enumerate}
    \item Minimal damping (underdamped, high oscillation)
    \item Moderate damping (underdamped, visible overshoot)
    \item High damping (near critical, minimal overshoot)
    \item Maximum damping (overdamped, no oscillation)
\end{enumerate}
...
\begin{enumerate}
    \item Why does $\omega_n$ remain approximately constant across damping settings?

    \item What practical limitations prevent achieving exactly $\zeta = 1$ (critical damping)?

    \item How does the friction-based damper differ from ideal viscous damping? What effect does this have on the response shape?

    \item If you wanted to double the natural frequency without changing the damping ratio, what would you change?

    \item Compare your measured settling times with the theoretical prediction $t_s = 4/(\zeta\omega_n)$. What sources of error might explain discrepancies?
\end{enumerate}
...
A control system must meet the following specifications:
\begin{itemize}
    \item Settling time: $t_s \leq 2\,\text{s}$
...
\label{ex:sysid}
The following step response data was measured from an unknown second-order system:
\begin{itemize}
    \item Final (steady-state) value: $y_{ss} = 2.0$
    \item First peak value: $y_{max} = 2.52$ at $t_p = 0.25\,\text{s}$
    \item Second peak value: $y_2 = 2.20$ at $t = 0.5\,\text{s}$
\end{itemize}
...
\textbf{Summary:}
\begin{itemize}
    \item First-order mode: pole at $s_1 = -5$ (time constant $\tau_1 = 0.2\,\text{s}$)
    \item Second-order mode: poles at $s_{2,3} = -2 \pm j4$ ($\zeta = 0.447$, $\omega_n = 4.47\,\text{rad/s}$)
\end{itemize}
...
\begin{itemize}
    \item Real part of $s_1$: $-5$
    \item Real part of $s_{2,3}$: $-2$
\end{itemize}
