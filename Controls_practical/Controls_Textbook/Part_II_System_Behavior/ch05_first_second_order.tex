...
\textbf{Data Acquisition Algorithm:}

\begin{algorithm}[H]
\caption{Second-Order System Distance Measurement}
\begin{algorithmic}[1]
\State \textbf{Configuration:}
\State \quad Set trigger pin $\leftarrow$ output pin 9
\State \quad Set echo pin $\leftarrow$ input pin 10
\State \quad Set serial baud rate $\leftarrow$ 115200
\State
\State \textbf{Initialization:}
\State Record start time $t_0 \leftarrow$ current time in milliseconds
\State
\State \textbf{Main Loop:}
\While{data acquisition active}
    \State $d \leftarrow$ \Call{MeasureDistance}{ }
    \State $t_{elapsed} \leftarrow$ current time $-$ $t_0$
    \State Output $(t_{elapsed}, d)$ to serial port
    \State Wait 10~ms \Comment{100~Hz sampling rate}
\EndWhile
\State
\Function{MeasureDistance}{ }
    \State Set trigger pin LOW
    \State Wait 2~$\mu$s
    \State Set trigger pin HIGH
    \State Wait 10~$\mu$s
    \State Set trigger pin LOW
    \State $t_{pulse} \leftarrow$ measure echo pulse duration
    \State \Return $t_{pulse} \times 0.034 / 2$ \Comment{distance in cm}
\EndFunction
\end{algorithmic}
\end{algorithm}
...
%-----------------------------------------------------------------------------
\begin{examplebox}[Time Constant from RC Circuit Values]
...
%-----------------------------------------------------------------------------
\begin{examplebox}[Finding $\zeta$ and $\omega_n$ from Transfer Function]
...
\textbf{Key Times:} At $t = \tau$, the output reaches 63.2\% of the final value. The rise time (10\%--90\%) is $t_r = 2.2\tau$, and the settling time (2\%) is $t_s = 4\tau$.
\end{keyconceptbox}
...
\textbf{Underdamped Response} ($\zeta < 1$): The damped frequency is $\omega_d = \omega_n\sqrt{1-\zeta^2}$, the percent overshoot is $PO = e^{-\pi\zeta/\sqrt{1-\zeta^2}} \times 100\%$, the peak time is $t_p = \pi/\omega_d$, and the settling time is $t_s = 4/(\zeta\omega_n)$.
...
\begin{exercise}
An RC circuit has $R = 47\,\text{k}\Omega$ and $C = 10\,\mu\text{F}$. Calculate the time constant, the frequency at which the output is attenuated by 3~dB, and the time required for the output to reach 99\% of its final value after a step input.
\end{exercise}

\begin{exercise}
A mass-spring-damper system has $m = 2\,\text{kg}$, $k = 50\,\text{N/m}$, and $c = 8\,\text{N}\cdot\text{s/m}$. Determine $\omega_n$, $\zeta$, and classify the system as underdamped, critically damped, or overdamped.
\end{exercise}

\begin{exercise}
For the transfer function $G(s) = 25/(s^2 + 3s + 25)$:
\begin{enumerate}[label=(\alph*)]
...
\begin{exercise}
Design a second-order system (specify $\zeta$ and $\omega_n$) that meets the following specifications: settling time $\leq 0.5\,\text{s}$ and percent overshoot $\leq 5\%$.
\end{exercise}

\begin{exercise}
A step response measurement shows the following: initial value 0, final value 4.0, first peak 5.6 at $t = 0.1\,\text{s}$. Assuming a second-order system, estimate the transfer function.
\end{exercise}

\begin{exercise}
For the system $G(s) = 10/[(s+1)(s+10)]$, determine whether first-order or second-order behavior dominates the step response. Justify your answer and estimate the settling time.
\end{exercise}

\begin{exercise}
An automobile suspension is modeled as a second-order system with $\omega_n = 8\,\text{rad/s}$. What damping ratio provides 20\% overshoot? If the sprung mass is 400~kg, calculate the required spring constant and damping coefficient.
\end{exercise}

\begin{exercise}
Prove that for an underdamped second-order system, the logarithmic decrement $\delta = \ln(x_n/x_{n+1})$ is related to the damping ratio by $\zeta = \delta/\sqrt{4\pi^2 + \delta^2}$.
\end{exercise}

\begin{exercise}
A seismograph has $\omega_n = 1\,\text{rad/s}$ and $\zeta = 0.7$. An earthquake produces ground acceleration $a(t) = A\sin(\omega t)$. For what range of earthquake frequencies $\omega$ will the seismograph output amplitude be within 10\% of the true ground displacement amplitude?
\end{exercise}

\begin{exercise}
Consider the third-order system $G(s) = 50/[(s+1)(s^2+4s+25)]$. Identify all poles, determine which are dominant, and estimate the settling time and percent overshoot of the step response.
\end{exercise}
