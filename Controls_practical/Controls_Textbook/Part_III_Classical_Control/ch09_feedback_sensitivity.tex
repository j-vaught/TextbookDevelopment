...
\begin{historicalbox}[Harold S. Black and the Negative Feedback Amplifier]
...
The breakthrough came on the morning of August 2, 1927, during Black's commute on the Lackawanna Ferry crossing the Hudson River from New Jersey to Manhattan. In a flash of insight, Black conceived of an entirely different approach: rather than trying to build a perfect amplifier, he would use feedback to force an imperfect amplifier to behave as if it were nearly ideal. Black's patent application, filed in 1928, spent nine years in the Patent Office before U.S. Patent 2,102,671 was finally granted on December 21, 1937. The patent examiner's skepticism was understandable---why would an engineer deliberately ``throw away'' the hard-won gain that vacuum tube designers had struggled for years to achieve?
\end{historicalbox}
...
This reveals the fundamental tradeoffs inherent in feedback design. First, making $T$ large for good tracking simultaneously amplifies the measurement noise $N$. Second, making $S$ small for effective disturbance rejection requires high loop gain, which in turn makes $T$ large. Third, at high frequencies where we want $T$ small to reject noise, the constraint forces $S \to 1$, meaning disturbance rejection is inevitably lost at these frequencies. The art of control design lies in managing these tradeoffs across the frequency spectrum.
...
\begin{tikzpicture}[scale=0.9]
    ...
    % Motor driver
    \draw[fill=red!20] (5,0) rectangle (8,2);
    ...
    % Brake
    \draw[fill=orange!30] (12,2.2) rectangle (14,3.2);
    ...
    % Power supply
    \draw[fill=gray!20] (5,-2) rectangle (8,-1);
...
The following algorithm implements both open-loop and closed-loop control modes, allowing direct comparison. This pseudocode can be adapted to any microcontroller platform.

\begin{algorithm}
\caption{Motor Speed Control with Disturbance Rejection}
\begin{algorithmic}[1]
\State \textbf{Constants:} $CPR \gets 64$, $GEAR\_RATIO \gets 50$, $SAMPLE\_TIME \gets 50$ ms
\State \textbf{Parameters:} $K_p \gets 2.0$, $K_i \gets 5.0$, $setpoint \gets 60.0$ RPM
\State \textbf{Variables:} $encoderCount \gets 0$, $integral \gets 0$, $closedLoop \gets \texttt{true}$
\State
...
\State \hspace{1em} Set motor direction forward
\State
...
\While{system is running}
    \State Check for mode switch commands from serial input
    \If{command received is `o'}
        \State $closedLoop \gets \texttt{false}$; $integral \gets 0$
    \ElsIf{command received is `c'}
        \State $closedLoop \gets \texttt{true}$
    \EndIf
    \State
    \If{$SAMPLE\_TIME$ has elapsed since last sample}
        \State Disable interrupts temporarily
        \State $count \gets encoderCount$; $encoderCount \gets 0$
        \State Enable interrupts
        \State $dt \gets$ elapsed time in seconds
        \State $rpm \gets (count \times 60) / (CPR \times GEAR\_RATIO \times dt)$
        \State
        \If{$closedLoop$}
            \State $error \gets setpoint - rpm$
            \State $integral \gets integral + error \times dt$
            \State $integral \gets \Call{clamp}{integral, -50, 50}$
            \State $output \gets K_p \times error + K_i \times integral$
            \State $pwmOut \gets \Call{clamp}{128 + output, 0, 255}$
        \Else
            \State $pwmOut \gets 128$ \Comment{Fixed open-loop value}
        \EndIf
        \State
        \State Write $pwmOut$ to PWM output
        \State Output setpoint, speed, and PWM values for plotting
    \EndIf
\EndWhile
\end{algorithmic}
\end{algorithm}
...
\begin{examplebox}[Sensitivity Calculation: Open-Loop vs. Closed-Loop]
...
\begin{examplebox}[Disturbance Rejection Analysis]
...
\begin{tikzpicture}[auto, node distance=2.2cm, >=latex', scale=0.85, transform shape]
    ...
    \draw[dashed, thick] (-2,-4) rectangle (13,3.2);

    % Key equations box
    \node[draw, fill=yellow!20, text width=5.5cm, align=left, font=\small] at (10.5,-3.5) {
...
\begin{exercise}
A unity feedback system has $G(s) = K/(s+5)$ and $C(s) = 10$.
\begin{enumerate}[(a)]
...
\begin{exercise}
For the system in Exercise 1, a step disturbance of magnitude 2 enters at the plant input. Find:
\begin{enumerate}[(a)]
...
\begin{exercise}
An operational amplifier with $A_{OL} = 50{,}000$ is used in an inverting configuration with $R_1 = \SI{1}{k\Omega}$ and $R_f = \SI{100}{k\Omega}$.
\begin{enumerate}[(a)]
...
\begin{exercise}
Derive the transfer function from measurement noise $N(s)$ to output $Y(s)$ for the standard feedback configuration. Explain why high bandwidth (high $|T(j\omega)|$ at high frequencies) is undesirable from a noise perspective.
\end{exercise}

\begin{exercise}
A cruise control system has plant $G(s) = 1/(10s+1)$ representing vehicle dynamics. Design a PI controller $C(s) = K_p + K_i/s$ such that:
\begin{enumerate}[(a)]
...
\begin{exercise}
Prove that for a general feedback system, the sensitivity of $T$ with respect to the feedback element $H$ is:
\begin{equation}
S^H_T = -\frac{L}{1+L} = -T
\end{equation}
...
\begin{exercise}
A temperature control system has plant $G(s) = 2/((s+0.1)(s+1))$. The plant gain can vary by $\pm 30\%$ due to heat exchanger fouling. Design a proportional controller to limit the closed-loop gain variation to $\pm 3\%$.
\end{exercise}

\begin{exercise}
Using the Bode magnitude plot, sketch $|S(j\omega)|$ and $|T(j\omega)|$ for:
\begin{equation}
...
\begin{exercise}
In the motor control experiment (Section~\ref{sec:practical_experiment}), derive a theoretical expression for the disturbance rejection ratio as a function of proportional gain $K_p$, assuming the motor transfer function is $G(s) = K_m/(s + a)$.
\end{exercise}

\begin{exercise}
\textbf{Design project:} Implement the motor speed control experiment using an Arduino and any available DC motor with encoder. Measure the disturbance rejection ratio for at least 4 different controller gains. Plot DRR versus loop gain and compare with theoretical predictions.
\end{exercise}

\begin{exercise}
Consider a feedback system where the plant transfer function is $G(s) = \dfrac{5}{(s+1)(s+5)}$ and a proportional controller $C(s) = K_p$ is used with unity feedback. Calculate the steady-state error to a unit step input for $K_p = 1, 10, 100$ and explain the trend.
\end{exercise}

\begin{exercise}
For the standard feedback configuration, show that when the loop gain $|L(j\omega)| \gg 1$, the sensitivity function magnitude $|S(j\omega)| \approx 1/|L(j\omega)|$. Use this to explain why high loop gain at low frequencies is desirable for disturbance rejection.
\end{exercise}
