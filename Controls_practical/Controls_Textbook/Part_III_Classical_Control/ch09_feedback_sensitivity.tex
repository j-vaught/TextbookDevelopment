...
The negative feedback amplifier transformed telecommunications. By 1937, transcontinental telephone service with acceptable quality became routine. The same principle was applied to:

\begin{itemize}
\item Radio transmitters and receivers
\item Public address systems
\item Recording and playback equipment
\item Early electronic computers (operational amplifiers)
\item Servo mechanisms for radar tracking and gun control (World War II)
\end{itemize}
...
Consider a DC motor speed control system with:
\begin{itemize}
\item Plant: $G(s) = 10/(s+1)$ (motor dynamics)
\item Controller: $C(s) = 5$ (proportional gain)
\item Unity feedback: $H(s) = 1$
\end{itemize}
...
\begin{itemize}
\item When $|S(j\omega)|$ is small (good disturbance rejection), $|T(j\omega)| \approx 1$ (good tracking).
\item When $|T(j\omega)|$ is small (poor tracking), $|S(j\omega)| \approx 1$ (poor disturbance rejection).
\item We cannot have both $|S|$ and $|T|$ small simultaneously at any frequency.
\end{itemize}
...
The constraint $S + T = 1$ means that feedback design involves allocating the ``sensitivity budget'' across frequency:

\begin{itemize}
\item \textbf{Low frequencies:} Make $|S| \ll 1$ for disturbance rejection and reference tracking accuracy. This requires high loop gain $|L| \gg 1$.

\item \textbf{High frequencies:} Make $|T| \ll 1$ to attenuate sensor noise. This requires low loop gain $|L| \ll 1$.

\item \textbf{Crossover region:} The transition between high and low loop gain must be designed carefully to ensure stability (phase margin $> 30°$, gain margin $> 6$ dB).
\end{itemize}
...
\begin{itemize}
\item DC motor with optical encoder (e.g., Pololu 37D metal gearmotor with 64 CPR encoder)
\item Arduino Uno or compatible microcontroller
\item L298N dual H-bridge motor driver
\item 12V DC power supply
\item Adjustable friction brake (described below)
\item Multimeter with tachometer function (optional)
...
A control system has the following nominal parameters:
\begin{itemize}
\item Controller: $C = 20$
\item Plant: $G = 5$ (nominal), range $[3, 8]$
\item Sensor: $H = 1$
\end{itemize}
...
\textbf{Design choice:} $K_c = 15$ provides margin. This gives:
\begin{itemize}
\item Loop gain: $L(0) = 30$
\item Sensitivity: $S = 1/31 = 0.032$
\item Closed-loop variation: $0.032 \times 25\% = 0.8\% < 1\%$ \checkmark
\end{itemize}
\end{example}
...
\label{ex:opamp}
An operational amplifier has open-loop gain $A_{OL} = 10^5$ with the following variations:
\begin{itemize}
\item Temperature coefficient: $-3000$ ppm/°C
\item Operating range: $0°C$ to $70°C$ (standard commercial)
\end{itemize}
...
\item \textbf{Sensitivity function:} The function $S(s) = 1/(1+L(s))$ quantifies both:
\begin{itemize}
    \item The attenuation of disturbances entering at the plant input
    \item The reduction in sensitivity to plant parameter variations
\end{itemize}
