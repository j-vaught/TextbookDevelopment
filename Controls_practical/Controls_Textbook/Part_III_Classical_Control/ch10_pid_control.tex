...
The gap between engineering practice and theoretical understanding was bridged by Nicolas Minorsky, a Russian-American engineer working for the United States Navy. Minorsky had been tasked with improving the automatic steering systems used on naval vessels, and he approached the problem with a mathematician's rigor combined with an engineer's practical sensibility.

In his landmark 1922 paper, ``Directional Stability of Automatically Steered Bodies,'' published in the \textit{Journal of the American Society of Naval Engineers}, Minorsky provided the first complete theoretical analysis of what we now call PID control \cite{minorsky1922}. His key insight came from observing how experienced helmsmen steered ships.

Minorsky noted that skilled helmsmen did not simply apply a rudder correction proportional to the current heading error. Instead, they:

\begin{enumerate}[label=(\arabic*)]
    \item Applied a correction proportional to the current error (P action)
    \item Applied additional correction when the error persisted over time (I action)
    \item Anticipated where the ship was heading based on how fast the error was changing (D action)
\end{enumerate}

This observation led Minorsky to propose the three-term controller:
\begin{equation}
\delta(t) = \Kp e(t) + \Ki \int_0^t e(\tau) \, d\tau + \Kd \frac{de(t)}{dt}
\label{eq:minorsky_pid}
\end{equation}

Minorsky's analysis was groundbreaking not just for proposing this controller structure, but for providing a mathematical framework to understand why each term was necessary:

\begin{itemize}
    \item \textbf{Proportional term}: Provides the primary corrective action. Larger errors produce larger corrections. However, for a stable equilibrium with non-zero external disturbance, the system must maintain some error to generate the necessary restoring force.

    \item \textbf{Integral term}: Accumulates error over time. Even a small persistent error will eventually produce a large integral, driving the system to eliminate the steady-state offset. Minorsky recognized this as the key to achieving zero steady-state error.

    \item \textbf{Derivative term}: Responds to the rate of change of error. When the ship is rapidly approaching the desired heading, the derivative term applies a ``braking'' action, reducing overshoot and oscillation.
\end{itemize}
...
Today, surveys consistently show that PID controllers constitute approximately 90--95\% of all control loops in industrial settings. This remarkable dominance is not because PID is optimal---more sophisticated control strategies exist---but because PID offers an unmatched combination of:

\begin{enumerate}[label=(\arabic*)]
    \item \textbf{Simplicity}: Three parameters capture the essential dynamics of most control problems
    \item \textbf{Robustness}: PID controllers tolerate significant model uncertainty
    \item \textbf{Tunability}: Operators can adjust behavior without deep theoretical knowledge
    \item \textbf{Universality}: The same algorithm works for temperature, pressure, flow, level, speed, and countless other variables
    \item \textbf{Maturity}: Nearly a century of engineering experience has produced reliable tuning methods
\end{enumerate}

As we develop the mathematical theory in subsequent sections, keep in mind that PID's endurance comes not from mathematical elegance alone, but from its practical effectiveness across an enormous range of applications.
...
The thermostat in your home or office implements PID control to maintain comfortable temperatures. Modern smart thermostats like the Nest or Ecobee use sophisticated digital PID algorithms with additional features such as:

\begin{itemize}
    \item \textbf{Adaptive tuning}: The controller learns the thermal characteristics of your building
    \item \textbf{Predictive heating/cooling}: Anticipates when to start conditioning based on learned patterns
    \item \textbf{Anti-windup}: Prevents integral term from accumulating when heating/cooling is saturated
\end{itemize}
...
Multirotor drones (quadcopters, hexacopters, etc.) rely heavily on PID control for stabilization. A typical drone runs multiple cascaded PID loops:

\begin{itemize}
    \item \textbf{Rate controllers}: PID loops controlling angular velocity around roll, pitch, and yaw axes (inner loop, running at 1--8 kHz)
    \item \textbf{Attitude controllers}: PID loops controlling orientation angles (outer loop, running at 250--1000 Hz)
    \item \textbf{Position controllers}: PID loops for GPS hold and waypoint navigation (running at 10--50 Hz)
\end{itemize}

The cascaded structure allows each loop to be tuned independently, and the fast inner loops provide stability while the slower outer loops provide trajectory tracking.
...
\begin{itemize}
    \item Oxygen sensors measure exhaust composition
    \item PID controller adjusts fuel injector pulse width
    \item Integral action ensures zero steady-state error in average air-fuel ratio
    \item Derivative action (often filtered) provides fast transient response
\end{itemize}
...
This reveals that the PID controller has:
\begin{itemize}
    \item One pole at $s = 0$ (from the integral action)
    \item Two zeros (from the numerator polynomial)
\end{itemize}
...
\textbf{Frequency domain perspective:} The derivative $\Kd s$ has magnitude $|\Kd s| = \Kd \omega$ increasing with frequency. This provides:
\begin{itemize}
    \item \textbf{Phase lead}: The phase contribution $+90°$ at all frequencies can improve phase margin
    \item \textbf{Increased bandwidth}: High-frequency response is amplified
\end{itemize}
...
Not all applications require all three terms. Common reduced-order controllers include:

\textbf{P Controller:} $C(s) = \Kp$
\begin{itemize}
    \item Simplest form; used when steady-state error is acceptable
    \item Applications: non-critical level control, manual-like operation
\end{itemize}

\textbf{PI Controller:} $C(s) = \Kp + \dfrac{\Ki}{s} = \Kp\left(1 + \dfrac{1}{\Ti s}\right)$
\begin{itemize}
    \item Most common industrial controller
    \item Eliminates steady-state error without noise amplification
    \item Applications: flow control, pressure control, slow temperature loops
\end{itemize}

\textbf{PD Controller:} $C(s) = \Kp + \Kd s = \Kp(1 + \Td s)$
\begin{itemize}
    \item Improves transient response without affecting steady-state
    \item Rarely used alone due to steady-state error
    \item Applications: systems where overshoot is critical and error is tolerable
\end{itemize}

\textbf{PID Controller:} Full three-term controller
\begin{itemize}
    \item Used when both steady-state accuracy and fast transient response are required
    \item Applications: precise temperature control, position control, process control
\end{itemize}
...
Our experimental setup controls the temperature of a small thermal mass (an aluminum block or water bath) using a resistive heating element. The system components are:

\begin{itemize}
    \item \textbf{Microcontroller}: Arduino Uno (or compatible)
    \item \textbf{Temperature sensor}: 10k$\Omega$ NTC thermistor (or TMP36 analog sensor)
    \item \textbf{Heating element}: 10$\Omega$, 10W power resistor
    \item \textbf{Driver}: N-channel MOSFET (e.g., IRF520) for PWM control
    \item \textbf{Thermal mass}: Small aluminum block (approximately $30 \times 30 \times 10$ mm)
    \item \textbf{Power supply}: 12V, 2A DC supply
\end{itemize}
...
The following Arduino code implements P, PI, and PID control with selectable modes:

\begin{lstlisting}[style=arduino, caption={Arduino PID Temperature Controller}]
// PID Temperature Controller
// Demonstrates P, PI, and PID control modes
...
float setpoint = 50.0;  // Target temperature (C)

// Control mode: 0=P, 1=PI, 2=PID
int controlMode = 2;
...
void setup() {
  ...
  Serial.println("Time(ms),Setpoint,Temperature,Output,Error");
}
...
float computePID(float temperature) {
...
void loop() {
  ...
  float output = computePID(temperature);
...
Before implementing control, understand your plant:
\begin{enumerate}
    \item Apply a constant PWM (e.g., 128 = 50\% duty cycle)
    \item Record temperature versus time
    \item Identify the time constant $\tau$ (time to reach 63.2\% of final value)
    \item Identify the steady-state gain $K$ (temperature rise per unit PWM)
\end{enumerate}

\textbf{Step 2: P-Only Control}

\begin{enumerate}
    \item Set \texttt{controlMode = 0}
    \item Start with low $\Kp$ (e.g., 10) and increase gradually
    \item Observe the steady-state error (offset from setpoint)
    \item Note: Higher $\Kp$ reduces but never eliminates the offset
\end{enumerate}

\textbf{Step 3: PI Control}

\begin{enumerate}
    \item Set \texttt{controlMode = 1}
    \item Keep $\Kp$ from Step 2
    \item Start with small $\Ki$ (e.g., 0.1) and increase
    \item Observe the steady-state error approach zero
    \item Note: Too much $\Ki$ causes overshoot and oscillation
\end{enumerate}

\textbf{Step 4: PID Control}

\begin{enumerate}
    \item Set \texttt{controlMode = 2}
    \item Keep $\Kp$ and $\Ki$ from previous steps
    \item Start with small $\Kd$ (e.g., 5) and increase
    \item Observe reduced overshoot and faster settling
    \item Note: Too much $\Kd$ may cause high-frequency oscillation (noise sensitivity)
\end{enumerate}
...
\begin{example}[Steady-State Error with P-Only Control]
...
\begin{example}[Integral Action Eliminates Steady-State Error]
...
For $L(s) = 5\Kp/s$:
\begin{itemize}
    \item Phase: $\angle L(j\omega) = -90°$ for all $\omega$
    \item Phase margin: $PM = 180° - 90° = 90°$ (satisfies requirement)
\end{itemize}
...
Verification:
\begin{itemize}
    \item Steady-state error: Zero (Type 1 system)
    \item Phase margin: 90° (exceeds 60° requirement)
    \item Gain crossover: 5 rad/s
\end{itemize}
\end{example}
...
This chapter has provided a comprehensive treatment of PID control, from its historical origins to modern implementation. The key concepts are:

\begin{enumerate}
    \item \textbf{Historical Development}: PID control evolved from practical ship steering problems (Sperry, 1911) through rigorous theoretical analysis (Minorsky, 1922) to widespread industrial adoption via pneumatic controllers (1930s--1940s).

    \item \textbf{The PID Control Law}:
    \begin{equation*}
    u(t) = \Kp e(t) + \Ki \int_0^t e(\tau) d\tau + \Kd \frac{de(t)}{dt}
    \end{equation*}

    \item \textbf{Role of Each Term}:
    \begin{itemize}
        \item Proportional: Fast response, but cannot eliminate steady-state error
        \item Integral: Eliminates steady-state error, but may cause overshoot
        \item Derivative: Provides damping and anticipation, but amplifies noise
    \end{itemize}

    \item \textbf{Practical Considerations}: Derivative filtering, anti-windup, and digital implementation are essential for real-world applications.

    \item \textbf{PID Dominance}: The combination of simplicity, robustness, and versatility explains why PID controllers constitute over 90\% of all industrial control loops.
\end{enumerate}
...
\begin{enumerate}
    \item A temperature control system has plant transfer function $G(s) = \dfrac{2}{5s + 1}$. Design a PI controller such that the closed-loop system has a damping ratio of 0.7 and zero steady-state error to step inputs.

    \item For the system in Problem 1, add derivative action to achieve a settling time of less than 3 seconds while maintaining zero steady-state error.

    \item Prove that a Type 0 plant with P-only control always has non-zero steady-state error for step inputs, regardless of the proportional gain value.

    \item A quadrotor altitude controller uses the transfer function $G(s) = \dfrac{1}{s^2}$ (double integrator). Explain why P-only control is marginally stable and design a PD controller for stable operation with damping ratio $\zeta = 0.707$.

    \item Using the Routh-Hurwitz criterion, determine the range of $\Ki$ for which the PI-controlled system with $G(s) = \dfrac{1}{s(s+1)(s+3)}$ and $\Kp = 2$ is stable.

    \item Implement a digital PID controller for an Arduino-based motor speed control system. The sampling rate is 100 Hz, and the desired response should have no more than 10\% overshoot with settling time under 0.5 seconds. Document your tuning procedure.

    \item Research and explain three different anti-windup strategies for integral action. Compare their effectiveness for systems with significant actuator saturation.

    \item The Ziegler-Nichols tuning method (covered in Chapter 11) provides initial PID gains based on critical gain and period. For a system with critical gain $K_u = 10$ and critical period $T_u = 2$ s, calculate the Ziegler-Nichols PID parameters and discuss their expected performance characteristics.
\end{enumerate}
