...
\begin{itemize}[noitemsep]
    \item Understand the historical development and industrial motivation for MPC
    \item Formulate MPC optimization problems for linear systems
    \item Convert MPC problems to standard quadratic programming form
    \item Implement receding horizon control with constraint handling
    \item Compare MPC performance to classical PID control under constraints
\end{itemize}
...
In the mid-1970s, Charles Cutler and Brian Ramaker at Shell developed \textbf{Dynamic Matrix Control (DMC)}, the first commercially successful MPC algorithm. Their key innovations were:

\begin{enumerate}
    \item \textbf{Step Response Models}: Rather than requiring detailed first-principles models, DMC used empirically measured step response coefficients. Engineers could identify these models by simply stepping each input and recording the output responses---no differential equations required.

    \item \textbf{Prediction over a Horizon}: The algorithm predicted future outputs over a ``prediction horizon'' of $N$ time steps, computing how each candidate control move would affect future behavior.

    \item \textbf{Move Suppression}: To prevent excessive control action (which wastes energy and stresses equipment), DMC penalized the magnitude of control changes, not just tracking errors.

    \item \textbf{Constraint Handling}: Most critically, DMC formulated control as a constrained optimization problem, directly incorporating actuator limits and output bounds.
\end{enumerate}
...
\begin{itemize}
    \item \textbf{1960s}: Mainframe computers filled rooms, cost millions of dollars, and could perform perhaps $10^5$ floating-point operations per second. Solving even a modest QP would require minutes to hours.

    \item \textbf{1970s}: Minicomputers like the DEC PDP-11 brought computing to industrial plants at costs under \$50,000. With $10^6$ FLOPS, a QP could be solved in seconds---fast enough for slow chemical processes with sampling times of minutes.

    \item \textbf{1980s--1990s}: Workstations and embedded processors reached $10^7$--$10^9$ FLOPS, enabling MPC for faster systems (sampling times of seconds) and larger problems.

    \item \textbf{2000s--present}: Modern microprocessors exceeding $10^{11}$ FLOPS enable MPC for automotive, aerospace, and robotics applications with millisecond sampling times.
\end{itemize}
...
\begin{enumerate}
    \item \textbf{Stability through Terminal Constraints}: David Mayne, James Rawlings, and others showed that adding a ``terminal constraint''---requiring the predicted state to reach a target set by the end of the horizon---guaranteed closed-loop stability under mild conditions.

    \item \textbf{Stability through Terminal Cost}: Alternatively, adding a ``terminal cost'' term to the objective function (typically an infinite-horizon LQR cost for the final state) provided stability without explicit terminal constraints.

    \item \textbf{Robust MPC}: Extensions to handle model uncertainty, ensuring stability even when the true plant differs from the prediction model.

    \item \textbf{Explicit MPC}: For systems with modest state and horizon dimensions, the optimal control law could be computed offline as a piecewise-affine function of the state, eliminating online optimization entirely.
\end{enumerate}
...
\begin{itemize}
    \item \textbf{1978--1985}: Early adopters at major oil companies (Shell, Exxon, Texaco) deployed custom systems on high-value units.

    \item \textbf{1985--1995}: Commercial MPC software (DMC-Plus, RMPCT, Connoisseur) made the technology accessible to smaller companies. Thousands of installations worldwide.

    \item \textbf{1995--2010}: MPC became the default advanced control technology for refineries, petrochemical plants, and other process industries. Integration with plant-wide optimization systems.

    \item \textbf{2010--present}: Extension beyond process industries to automotive (fuel economy optimization, emissions control), building systems (HVAC optimization), power electronics, and autonomous vehicles.
\end{itemize}
...
\begin{itemize}
    \item \textbf{Crude distillation}: Optimizing product cuts while satisfying quality specifications
    \item \textbf{Catalytic cracking}: Maximizing gasoline yield within temperature and catalyst constraints
    \item \textbf{Polymerization reactors}: Controlling molecular weight distributions
    \item \textbf{Batch processes}: Optimizing recipe trajectories for pharmaceutical and specialty chemical production
\end{itemize}
...
\begin{itemize}
    \item \textbf{Explicit constraint handling}: Vehicle dynamics, actuator limits, and collision avoidance expressed as optimization constraints
    \item \textbf{Preview capability}: The prediction horizon can ``see'' upcoming curves, intersections, and obstacles
    \item \textbf{Graceful degradation}: When perfect tracking is impossible (e.g., obstacle too close), MPC finds the best feasible compromise
\end{itemize}
...
\begin{itemize}
    \item \textbf{Weather prediction integration}: Using forecast data to pre-cool buildings before heat waves or pre-heat before cold snaps
    \item \textbf{Occupancy-aware control}: Reducing conditioning in unoccupied zones while maintaining comfort in occupied spaces
    \item \textbf{Demand response}: Shifting energy consumption to off-peak hours or responding to grid signals
    \item \textbf{Thermal storage}: Optimally charging and discharging ice storage or building thermal mass
\end{itemize}
...
\begin{itemize}
    \item \textbf{Trajectory optimization}: Computing dynamically feasible paths through cluttered environments
    \item \textbf{Constraint satisfaction}: Respecting motor speed limits, battery current constraints, and no-fly zones
    \item \textbf{Disturbance rejection}: Adapting to wind gusts and payload changes
    \item \textbf{Multi-vehicle coordination}: Coordinating swarms while avoiding inter-vehicle collisions
\end{itemize}
...
\begin{enumerate}
    \item \textbf{Feedback}: By re-solving at each step, MPC incorporates new measurements and rejects disturbances.
    \item \textbf{Finite computation}: Each optimization involves only $N$ time steps, not infinity.
    \item \textbf{Implicit robustness}: Model errors are partially corrected by feedback.
    \item \textbf{Adaptation}: Changing constraints or setpoints are automatically incorporated.
\end{enumerate}
...
where:
\begin{itemize}
    \item $\state_k \in \Real^n$ is the state vector at time step $k$
    \item $\inputvec_k \in \Real^m$ is the control input vector
    \item $\outputvec_k \in \Real^p$ is the output vector
    \item $\mat{A} \in \Real^{n \times n}$ is the state transition matrix
    \item $\mat{B} \in \Real^{n \times m}$ is the input matrix
    \item $\mat{C} \in \Real^{p \times n}$ is the output matrix
    \item $\mat{D} \in \Real^{p \times m}$ is the feedthrough matrix (often zero)
\end{itemize}
...
where:
\begin{itemize}
    \item $\mat{Q} \in \Real^{n \times n}$ is the state weighting matrix ($\mat{Q} \succeq 0$, positive semi-definite)
    \item $\mat{R} \in \Real^{m \times m}$ is the input weighting matrix ($\mat{R} \succ 0$, positive definite)
    \item $\mat{P} \in \Real^{n \times n}$ is the terminal state weighting matrix ($\mat{P} \succeq 0$)
    \item $N$ is the prediction horizon length
\end{itemize}

\textbf{Physical interpretation}:
\begin{itemize}
    \item The $\state^\top \mat{Q} \state$ terms penalize state deviation from zero (or from a reference trajectory)
    \item The $\inputvec^\top \mat{R} \inputvec$ terms penalize control effort (actuator usage, energy consumption)
    \item The terminal cost $\state_{k+N}^\top \mat{P} \state_{k+N}$ accounts for behavior beyond the horizon
    \item Large $\mat{Q}$ entries demand tight tracking; large $\mat{R}$ entries demand gentle control action
\end{itemize}
...
where:
\begin{itemize}
    \item $\mat{H} = \boldsymbol{\mathcal{B}}^\top \bar{\mat{Q}} \boldsymbol{\mathcal{B}} + \bar{\mat{R}} \in \Real^{Nm \times Nm}$ is the Hessian matrix (positive definite)
    \item $\vect{f} = \boldsymbol{\mathcal{B}}^\top \bar{\mat{Q}} \boldsymbol{\mathcal{A}} \state_k \in \Real^{Nm}$ is the linear cost vector
    \item $\mat{G}$ and $\vect{w}$ encode all inequality constraints
\end{itemize}
...
\textbf{Theorem (MPC Stability):} The MPC closed-loop system is asymptotically stable if:
\begin{enumerate}
    \item The terminal cost $\mat{P}$ satisfies the Lyapunov equation $\mat{A}^\top \mat{P} \mat{A} - \mat{P} + \mat{Q} = \mat{0}$ (i.e., $\mat{P}$ is the infinite-horizon LQR cost matrix), OR
    \item A terminal constraint $\state_{k+N} \in \mathcal{X}_f$ is imposed, where $\mathcal{X}_f$ is a control-invariant set containing the origin, AND
    \item The optimization problem is feasible at each time step.
\end{enumerate}
...
\textbf{Theorem (MPC-LQR Equivalence):} For linear systems with:
\begin{itemize}
    \item No constraints
    \item Infinite horizon ($N \to \infty$)
    \item Terminal cost $\mat{P}$ equal to the LQR cost matrix
\end{itemize}
...
\begin{itemize}
    \item LQR is the ``idealized'' case of MPC---no constraints, perfect model, infinite foresight
    \item MPC gracefully degrades from LQR as constraints become active
    \item When constraints are inactive, MPC behaves like LQR; when active, MPC finds the best feasible approximation
\end{itemize}
...
\begin{itemize}
    \item Tank walls: 3mm thickness minimum for water resistance
    \item Material: PETG recommended (water-resistant, food-safe)
    \item Include mounting points for sensors and tubes
    \item Consider o-ring grooves for leak-proof fittings
    \item Print time: approximately 4-6 hours per tank at 0.2mm layer height
\end{itemize}
...
\begin{lstlisting}[caption={PID Controller Implementation}, label={lst:pid_tank}]
...
\begin{lstlisting}[caption={Simplified MPC Implementation}, label={lst:mpc_tank}]
// Simplified MPC for two-tank system
// Uses explicit solution for small horizon

class SimpleMPC {
  // Model parameters (discrete-time)
  float A[2][2], B[2];  // State-space matrices
  float Q[2], R;        // Cost weights
  int N;                // Horizon length
  float uMin, uMax;     // Input constraints

  // Precomputed matrices for horizon N=3
  float H, f_coeff[2];  // QP parameters

public:
  SimpleMPC(float a11, float a12, float a21, float a22,
            float b1, float b2, float q1, float q2,
            float r, float umin, float umax) {
    A[0][0] = a11; A[0][1] = a12;
    A[1][0] = a21; A[1][1] = a22;
    B[0] = b1; B[1] = b2;
    Q[0] = q1; Q[1] = q2;
    R = r;
    uMin = umin; uMax = umax;
    N = 3;  // Fixed horizon for simplicity

    // Precompute H matrix for N=3 horizon
    // H = B'*Q_bar*B + R_bar (simplified scalar version)
    computeQPMatrices();
  }

  void computeQPMatrices() {
    // For educational purposes, using simplified computation
    // Full implementation would build the matrices from Sec III
    H = 3*R + (Q[0]*B[0]*B[0] + Q[1]*B[1]*B[1]) *
        (1 + A[0][0]*A[0][0] + A[0][0]*A[0][0]*A[0][0]*A[0][0]);

    f_coeff[0] = 2*(Q[0]*B[0]*A[0][0] + Q[1]*B[1]*A[1][0]);
    f_coeff[1] = 2*(Q[0]*B[0]*A[0][1] + Q[1]*B[1]*A[1][1]);
  }

  float compute(float x1, float x2) {
    // Compute linear cost term f = B'*Q_bar*A_bar*x
    float f = f_coeff[0]*x1 + f_coeff[1]*x2;

    // Unconstrained optimum: u* = -H^{-1} * f
    float u_opt = -f / H;

    // Project onto constraints [uMin, uMax]
    if (u_opt < uMin) u_opt = uMin;
    if (u_opt > uMax) u_opt = uMax;
...
// Main control loop
SimpleMPC mpc(/* parameters */);
PIDController pid(2.0, 0.5, 0.1, 0, 255);

void loop() {
  // Read sensor values
  float h1 = readLevel(SENSOR1_PIN);
  float h2 = readLevel(SENSOR2_PIN);

  // Compute control based on mode
  float u;
  if (useMPC) {
    // MPC: uses state (deviation from setpoint)
    float x1 = h1 - setpoint1;
    float x2 = h2 - setpoint2;
    u = mpc.compute(x1, x2);
  } else {
    // PID: single-loop on tank 2 level
    u = pid.compute(setpoint2, h2, dt);
  }

  // Apply control
  analogWrite(PUMP_PIN, (int)u);

  // Log data for analysis
  logData(h1, h2, u, setpoint2);
...
\begin{enumerate}
    \item Fill reservoir and establish steady-state at mid-level
    \item Apply step changes in pump PWM (e.g., 100 to 150)
    \item Record tank levels over time
    \item Fit first-order-plus-delay models to responses
    \item Extract time constants $T_1$, $T_2$ and gains
\end{enumerate}

\subsubsection{Part 2: PID Control Experiment}

\begin{enumerate}
    \item Tune PID controller for acceptable step response at nominal operating point
    \item Command a large positive setpoint change (level increase)
    \item Observe: pump saturates at maximum, but recovers reasonably
    \item Command a large negative setpoint change (level decrease)
    \item Observe: pump saturates at zero (cannot go negative), integrator winds up
    \item Record overshoot, settling time, and constraint violations
\end{enumerate}

\subsubsection{Part 3: MPC Control Experiment}

\begin{enumerate}
    \item Configure MPC with same target response speed as PID
    \item Repeat the large positive setpoint change
    \item Observe: MPC may preemptively limit pump rate to avoid later saturation
    \item Repeat the large negative setpoint change
    \item Observe: MPC reduces pump to minimum (not negative), plans descent trajectory
    \item Compare performance metrics to PID
\end{enumerate}
...
\subsection{Example 1: Setting Up MPC Matrices for a Double Integrator}
...
\subsection{Example 2: QP Formulation with Input Constraints}

\textbf{Problem}: For the double integrator from Example 1, formulate the complete QP with constraints $-1 \leq u_k \leq 1$ for all $k$.
...
\textbf{Observations}:
\begin{itemize}
    \item Short horizons ($N=1,2$) give less aggressive control---the controller ``doesn't see'' far enough
    \item As $N$ increases, control approaches the LQR solution
    \item Cost decreases with horizon length (more optimization freedom)
    \item Diminishing returns: $N=10$ captures most of the benefit
\end{itemize}
...
\begin{enumerate}
    \item \textbf{Historical Context}: MPC emerged from 1970s petrochemical control challenges. Shell's DMC and Richalet's MPHC provided practical algorithms before complete theoretical understanding existed. The key insight was using process models for prediction and optimization for constraint handling.

    \item \textbf{Modern Applications}: MPC now spans chemical processes, autonomous vehicles, building systems, and robotics---any domain requiring constraint-aware trajectory optimization.

    \item \textbf{Mathematical Foundation}: MPC solves finite-horizon optimal control problems at each sampling instant:
    \begin{itemize}
        \item Prediction model: $\state_{k+1} = \mat{A}\state_k + \mat{B}u_k$
        \item Cost function: quadratic penalties on states and inputs
        \item Constraints: input limits, state bounds, terminal conditions
        \item Receding horizon: solve, apply first input, repeat
    \end{itemize}

    \item \textbf{QP Formulation}: Eliminating state predictions converts MPC to standard quadratic programming:
    \begin{equation*}
        \min_{\mathbf{U}} \frac{1}{2}\mathbf{U}^\top\mat{H}\mathbf{U} + \vect{f}^\top\mathbf{U} \quad \text{s.t.} \quad \mat{G}\mathbf{U} \leq \vect{w}
    \end{equation*}

    \item \textbf{Stability}: Terminal cost or constraints ensure closed-loop stability. With appropriate terminal cost, unconstrained MPC equals LQR.

    \item \textbf{Practical Implementation}: The tank-level experiment demonstrated MPC's graceful constraint handling compared to PID's windup problems.
\end{enumerate}
...
\begin{enumerate}
    \item Explain why MPC re-solves the optimization problem at every time step rather than executing the entire precomputed control sequence.

    \item A colleague claims ``MPC is just optimal control, nothing new.'' Explain what distinguishes MPC from classical optimal control theory.

    \item Why does MPC require positive definite $\mat{R}$ but only positive semi-definite $\mat{Q}$?

    \item What happens to the MPC solution if the prediction model is perfect versus imperfect?
\end{enumerate}

\subsection{Computational Problems}

\begin{enumerate}[resume]
    \item For the system $\state_{k+1} = 0.9\state_k + 0.5u_k$ (scalar), with $Q=1$, $R=0.1$, $N=3$:
    \begin{enumerate}
    ...
    \item Repeat Problem 5 with input constraint $|u_k| \leq 0.5$. Show how the constraint affects the solution.

    \item For a double integrator with $T_s = 0.1$ s, design MPC parameters ($N$, $\mat{Q}$, $\mat{R}$) to achieve settling time approximately 1 second with input constraint $|u| \leq 10$.

    \item Prove that the Hessian matrix $\mat{H} = \boldsymbol{\mathcal{B}}^\top\bar{\mat{Q}}\boldsymbol{\mathcal{B}} + \bar{\mat{R}}$ is positive definite when $\mat{R} \succ 0$.
\end{enumerate}

\subsection{Analysis Problems}

\begin{enumerate}[resume]
    \item Consider MPC with horizon $N=1$ (one step ahead). Show that this reduces to a simple state feedback law. What is the feedback gain?

    \item For a marginally stable system ($\mat{A}$ has eigenvalues on the unit circle), explain why terminal constraints are particularly important for MPC stability.

    \item Derive the relationship between MPC and LQR when no constraints are active. Under what conditions are they identical?
\end{enumerate}

\subsection{Implementation Problems}

\begin{enumerate}[resume]
    \item Implement MPC in MATLAB/Python for a DC motor position control system. Compare performance with PID for:
    \begin{enumerate}
    ...
    \item Extend the tank-level MPC to include a constraint on maximum rate of change of pump speed (slew rate limiting).

    \item Implement an MPC controller for a simulated quadrotor hover task with thrust limits.
\end{enumerate}
...
\begin{itemize}
    \item J. M. Maciejowski, \textit{Predictive Control with Constraints}, Prentice Hall, 2002. [Excellent introduction with practical emphasis]

    \item J. B. Rawlings and D. Q. Mayne, \textit{Model Predictive Control: Theory and Design}, Nob Hill Publishing, 2009. [Comprehensive theoretical treatment]

    \item C. E. Garcia, D. M. Prett, and M. Morari, ``Model Predictive Control: Theory and Practice---A Survey,'' \textit{Automatica}, vol. 25, no. 3, pp. 335--348, 1989. [Classic survey paper]

    \item S. J. Qin and T. A. Badgwell, ``A Survey of Industrial Model Predictive Control Technology,'' \textit{Control Engineering Practice}, vol. 11, no. 7, pp. 733--764, 2003. [Industrial applications overview]

    \item F. Borrelli, A. Bemporad, and M. Morari, \textit{Predictive Control for Linear and Hybrid Systems}, Cambridge University Press, 2017. [Modern treatment with hybrid systems]
\end{itemize}
