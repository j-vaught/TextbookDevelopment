...
\begin{itemize}[leftmargin=2cm]
    \item Understand the historical development and motivation for adaptive control
    \item Master the mathematical foundations of Model Reference Adaptive Control (MRAC)
    \item Derive adaptation laws using both gradient descent (MIT Rule) and Lyapunov methods
    \item Implement self-tuning regulators and gain scheduling techniques
    \item Apply adaptive control to practical motor control problems
    \item Analyze stability and convergence properties of adaptive systems
\end{itemize}
...
\begin{keyconceptbox}[title=The MIT Rule Philosophy]
...
\begin{enumerate}
    \item \textbf{MIT Rule:} Start with an intuitive adaptation law, hope for stability
    \item \textbf{Lyapunov approach:} Start with a stability requirement, derive the adaptation law
\end{enumerate}
...
\begin{warningbox}[title=Rohrs' Counterexample]
Rohrs et al. demonstrated that adaptive controllers which were \textit{provably stable} under ideal assumptions could become \textit{unstable} when:
\begin{itemize}
    \item Unmodeled high-frequency dynamics were present
    \item Output disturbances were added
    \item Both conditions occurred together
\end{itemize}
...
\begin{enumerate}
    \item \textbf{Perfect model knowledge:} The plant was assumed to be exactly in the model class
    \item \textbf{No unmodeled dynamics:} High-frequency parasitic dynamics were ignored
    \item \textbf{No disturbances:} Proofs assumed noise-free measurements
\end{enumerate}
...
\begin{itemize}
    \item \textbf{$\sigma$-modification:} Adding a leakage term $-\sigma\theta$ to prevent parameter drift
    \item \textbf{$\varepsilon_1$-modification:} Dead-zone modifications to stop adaptation when error is small
    \item \textbf{Projection:} Constraining parameters to remain in a known bounded region
    \item \textbf{Normalization:} Scaling signals to prevent unbounded growth
\end{itemize}
...
\begin{itemize}
    \item \textbf{Commercial aircraft:} Fuel consumption changes aircraft mass by 30\% or more during long flights
    \item \textbf{Military aircraft:} Weapons release, battle damage, and configuration changes alter dynamics
    \item \textbf{Spacecraft:} Fuel depletion and appendage deployment change mass properties
    \item \textbf{UAVs:} Payload variations and icing conditions require adaptation
\end{itemize}
...
\begin{itemize}
    \item \textbf{Payload variation:} Robot arm carrying different loads requires different gains
    \item \textbf{Wear and degradation:} Joint friction and gear backlash change over time
    \item \textbf{Human-robot interaction:} Contact with humans requires adaptive compliance
    \item \textbf{Learning from demonstration:} Adapting to new tasks shown by operators
\end{itemize}
...
\begin{itemize}
    \item \textbf{Catalyst deactivation:} Reaction rates decrease over weeks or months
    \item \textbf{Fouling:} Heat exchanger coefficients degrade as deposits accumulate
    \item \textbf{Raw material variation:} Feed composition changes affect process dynamics
    \item \textbf{Seasonal changes:} Cooling water temperature affects heat transfer
\end{itemize}
...
\begin{itemize}
    \item \textbf{Artificial pancreas:} Insulin delivery must adapt to varying patient sensitivity
    \item \textbf{Anesthesia control:} Drug sensitivity varies between patients and during surgery
    \item \textbf{Prosthetics:} EMG-controlled prosthetic limbs adapt to user patterns
    \item \textbf{Rehabilitation robots:} Therapy robots adapt assistance to patient progress
\end{itemize}
...
\begin{itemize}
    \item \textbf{Engine control:} Fuel injection adapts to altitude, temperature, fuel quality
    \item \textbf{Transmission control:} Shift patterns adapt to driver behavior
    \item \textbf{Active suspension:} Damping adapts to road conditions and load
    \item \textbf{Autonomous vehicles:} Control adapts to road surface and conditions
\end{itemize}
...
\begin{keyconceptbox}[title=Reference Model Selection]
The reference model should satisfy:
\begin{enumerate}
    \item \textbf{Stability:} $A_m$ must have all eigenvalues in left half-plane
    \item \textbf{Achievability:} The plant must be capable of matching the model
    \item \textbf{Desired behavior:} Response should match specifications (rise time, overshoot, etc.)
\end{enumerate}
\end{keyconceptbox}
...
\begin{enumerate}
    \item Define the error dynamics
    \item Choose a Lyapunov function including both state error and parameter error
    \item Derive the adaptation law that makes $\dot{V} \leq 0$
\end{enumerate}
...
Consider the adaptive system with the adaptation law (\ref{eq:lyapunov_adaptation}). Then:
\begin{enumerate}
    \item All signals in the closed-loop system are bounded
    \item The tracking error $e(t) \to 0$ as $t \to \infty$
    \item The parameter estimates $\hat{\theta}(t)$ remain bounded
\end{enumerate}
\end{theorem}
...
\begin{enumerate}
    \item \textbf{Identify scheduling variable(s):} Measurable variable(s) that correlate with parameter changes (e.g., altitude, speed, temperature)

    \item \textbf{Define operating points:} Select representative conditions across the operating envelope

    \item \textbf{Design local controllers:} At each operating point, linearize and design an optimal controller

    \item \textbf{Implement interpolation:} Smoothly transition between controllers based on scheduling variable
\end{enumerate}
...
\begin{itemize}
    \item Operating points close enough that interpolated controllers stabilize intermediate plants
    \item Scheduling variable changes slowly compared to closed-loop dynamics
    \item Use of LPV (Linear Parameter-Varying) design methods that explicitly account for parameter rates
\end{itemize}
...
\begin{experimentbox}[title=Adaptive DC Motor Speed Control with Variable Load]
...
\begin{itemize}
    ...
    \item DC motor with encoder (e.g., Pololu 37D gearmotor with encoder)
    \item Motor driver (L298N H-bridge or similar)
    \item Power supply (12V, 2A recommended)
    \item Rotary disc with mounting points for weights
    \item Set of calibrated weights (10g, 20g, 50g, 100g)
    \item Oscilloscope or data logging capability
\end{itemize}
...
\begin{itemize}
    ...
    \item $J$: total moment of inertia (motor + load)
    \item $b$: viscous friction coefficient
...
\begin{lstlisting}[style=arduinoStyle, caption={Adaptive motor control Arduino code}]
// Adaptive Motor Speed Control
// Demonstrates MIT Rule adaptation with variable load
...
float gamma_p = 0.001;    // Adaptation rate for Kp
float gamma_i = 0.0005;   // Adaptation rate for Ki

// Reference model (first-order, tau = 0.2s)
float tau_m = 0.2;
float omega_m = 0.0;

// State variables
volatile long encoderCount = 0;
float omega = 0.0;        // Measured speed
...
void setup() {
    ...
    Serial.begin(115200);
    Serial.println("Time,Omega_ref,Omega_m,Omega,Kp,Ki,Error");
}

void loop() {
    unsigned long currentTime = millis();

    if (currentTime - lastTime >= (unsigned long)(dt * 1000)) {
        ...
        omega = (count * 2.0 * PI) / (1440.0 * dt);  // 1440 CPR

        // Update reference model
        omega_m += dt * (-omega_m / tau_m + omega_ref / tau_m);

        // Calculate errors
        error = omega_ref - omega;           // For PI controller
        tracking_error = omega - omega_m;    // For adaptation
        ...
        // MIT Rule adaptation
        Kp += gamma_p * tracking_error * error * dt;
        Ki += gamma_i * tracking_error * integral * dt;

        // Constrain gains to reasonable values
        Kp = constrain(Kp, 0.5, 10.0);
        Ki = constrain(Ki, 0.1, 5.0);
...
\begin{enumerate}
    \item \textbf{Baseline Characterization:}
    \begin{enumerate}
        \item Run motor with no load, record step response
        \item Identify time constant $\tau_0$ and gain $K_0$
        \item Tune fixed PI controller for good performance
    \end{enumerate}

    \item \textbf{Fixed Controller Under Load Variation:}
    \begin{enumerate}
        \item Set reference speed to 100 rad/s
        \item Wait for steady state
        \item Add 50g weight at 5cm radius
        \item Record response degradation
        \item Add 100g total weight
        \item Document further degradation
    \end{enumerate}

    \item \textbf{Adaptive Controller Under Load Variation:}
    \begin{enumerate}
        \item Reset to no load condition
        \item Enable adaptive control
        \item Repeat load variation sequence
        \item Record adaptation of gains
        \item Observe maintained performance
    \end{enumerate}

    \item \textbf{Comparative Analysis:}
    \begin{enumerate}
        \item Plot fixed vs. adaptive responses
        \item Calculate performance metrics (rise time, overshoot, settling time)
        \item Show gain adaptation history
    \end{enumerate}
\end{enumerate}
...
\begin{enumerate}
    \item Why does the proportional gain $K_p$ need to increase when inertia increases?
    \item What would happen if the adaptation gains $\gamma_p$ and $\gamma_i$ were too large?
    \item How would you modify the experiment to demonstrate the need for robustness modifications?
    \item Compare the experimental results with the theoretical predictions from Section \ref{sec:adaptive_math}.
\end{enumerate}
...
\begin{example}[MIT Rule for First-Order System]
\label{ex:mit_first_order}
...
\begin{example}[Lyapunov Adaptation Law Derivation]
\label{ex:lyapunov_derivation}
...
\begin{itemize}
    \item $V$ is bounded, so $e$, $\tilde{\theta}_1$, $\tilde{\theta}_2$ are bounded
    \item $e \in \mathcal{L}_2$ since $\int_0^\infty 4e^2 dt \leq V(0) < \infty$
    \item By Barbalat's lemma (since $\dot{e}$ is also bounded), $e(t) \to 0$ as $t \to \infty$
\end{itemize}
...
\begin{itemize}
    \item Overshoot increases significantly
    \item Risk of oscillation
\end{itemize}
...
\begin{itemize}
    \item Rise time increases by $\sim$40\%
    \item Settling time increases significantly
\end{itemize}
...
\begin{itemize}
    \item \textbf{Adaptive control} adjusts controller parameters online to handle unknown or time-varying plant dynamics

    \item \textbf{Model Reference Adaptive Control (MRAC)} makes the plant output track a reference model output

    \item The \textbf{MIT Rule} uses gradient descent to minimize tracking error: $\dot{\theta} = -\gamma e \frac{\partial e}{\partial \theta}$

    \item \textbf{Lyapunov-based adaptation} provides stability guarantees by deriving adaptation laws from Lyapunov functions

    \item \textbf{Self-Tuning Regulators} explicitly identify plant parameters and redesign the controller

    \item \textbf{Gain scheduling} interpolates between pre-designed controllers based on operating conditions

    \item \textbf{Robustness modifications} ($\sigma$-modification, dead zones, projection) prevent instability from unmodeled dynamics
\end{itemize}
...
\begin{enumerate}
    \item \textbf{Choose adaptation gains carefully:} Too high causes oscillation; too low causes slow tracking

    \item \textbf{Always include robustness modifications} in practical implementations

    \item \textbf{Ensure persistence of excitation} if parameter convergence is required

    \item \textbf{Normalize signals} to prevent large signal-induced instability

    \item \textbf{Project parameters} to known feasible regions

    \item \textbf{Start with gain scheduling} if operating points are well-defined; use full adaptation only when necessary

    \item \textbf{Test extensively} at off-nominal conditions before deployment
\end{enumerate}
...
\textbf{Problem 23.1:} A first-order plant $G(s) = \frac{2}{s+3}$ is controlled using MRAC with reference model $G_m(s) = \frac{5}{s+5}$.
\begin{enumerate}[(a)]
    \item Find the ideal controller parameters $\theta_1^*$ and $\theta_2^*$
    \item Derive the MIT Rule adaptation laws
    \item Simulate the system with $\gamma_1 = \gamma_2 = 1$ and initial conditions $\hat{\theta}_1(0) = 0$, $\hat{\theta}_2(0) = 0$
\end{enumerate}

\textbf{Problem 23.2:} For the system in Problem 23.1, derive the Lyapunov-based adaptation law and prove that $e(t) \to 0$.

\textbf{Problem 23.3:} Consider an aircraft with pitch dynamics:
\begin{equation}
...
\textbf{Problem 23.4:} Prove that the $\sigma$-modification guarantees ultimate boundedness of all signals even when the plant has bounded unmodeled dynamics.

\textbf{Problem 23.5:} Design a self-tuning PID controller for a second-order system using RLS identification. Implement the design in MATLAB/Simulink and compare with a fixed PID controller under parameter variations.

\textbf{Problem 23.6:} The following figure shows the response of an adaptive system:
...
\begin{enumerate}[(a)]
    \item What happened at $t = 4$s that caused the oscillation?
    \item What robustness modification would prevent this behavior?
    \item Sketch the expected response with your proposed modification
\end{enumerate}
...
\begin{itemize}
    ...
    \begin{itemize}
        \item \AA str\"om, K.J. and Wittenmark, B., \textit{Adaptive Control}, 2nd ed., Addison-Wesley, 1995
        \item Narendra, K.S. and Annaswamy, A.M., \textit{Stable Adaptive Systems}, Prentice Hall, 1989
        \item Ioannou, P.A. and Sun, J., \textit{Robust Adaptive Control}, Prentice Hall, 1996
    \end{itemize}

    \item \textbf{Historical papers:}
    \begin{itemize}
        \item Whitaker, H.P., Yamron, J., and Kezer, A., ``Design of Model Reference Adaptive Control Systems for Aircraft,'' MIT Instrumentation Laboratory Report R-164, 1958
        \item Rohrs, C.E., Valavani, L., Athans, M., and Stein, G., ``Robustness of Continuous-Time Adaptive Control Algorithms in the Presence of Unmodeled Dynamics,'' IEEE Trans. Automatic Control, 1985
    \end{itemize}

    \item \textbf{Modern developments:}
    \begin{itemize}
        \item Lavretsky, E. and Wise, K.A., \textit{Robust and Adaptive Control with Aerospace Applications}, Springer, 2013
        \item Hovakimyan, N. and Cao, C., \textit{L$_1$ Adaptive Control Theory}, SIAM, 2010
    \end{itemize}
\end{itemize}
