...
A transcontinental telephone call in 1940 passed through dozens of repeater amplifiers, each boosting the signal to compensate for cable losses. The system faced fundamental uncertainties from multiple sources. Component drift caused vacuum tube gains to vary by 20--30\% over their lifetime. Temperature dependence posed another challenge, as cables buried underground experienced seasonal temperature swings of 50°C or more, changing their attenuation characteristics. Manufacturing tolerances meant that no two amplifiers were identical, while load variations caused the number of simultaneous calls to affect impedance matching.
...
\begin{historicalbox}[Bode's Secret Wartime Work]
While developing his feedback amplifier theory at Bell Labs, Hendrik Bode was simultaneously engaged in classified wartime research. During World War II, he applied his feedback principles to fire-control systems for anti-aircraft guns---a problem where robustness to uncertainty was literally a matter of life and death. The targets were moving, the gun platform was pitching on a ship's deck, and the ammunition varied from round to round. Bode's insistence on adequate stability margins came directly from this experience: a system that worked perfectly on the test bench but failed in combat was worse than useless. This practical wartime experience profoundly shaped the conservative design philosophy that characterizes robust control to this day.
\end{historicalbox}
...
The MCAS design violated fundamental robust design principles in several critical ways. First, it created a single point of failure where one sensor controlled a critical system. Second, no uncertainty modeling was performed, meaning erroneous sensors were not considered in the design. Third, inadequate authority limits allowed MCAS to command unlimited nose-down trim. Finally, pilot override was inadequate because the system repeatedly reactivated after pilot correction.
...
\begin{tcolorbox}[colback=blue!5!white,colframe=blue!75!black,title=The Robustness Principle,sharp corners]
...
Modern aircraft operate across vast flight envelopes where aerodynamic characteristics change dramatically. Mach number effects present significant challenges, as transonic flight (Mach 0.8--1.2) exhibits complex shock-boundary layer interactions that defy precise modeling. Altitude variations cause air density to vary by orders of magnitude from sea level to cruise altitude, changing control effectiveness substantially. Fuel consumption during long flights can decrease aircraft mass by 40\%, while configuration changes from flaps, slats, landing gear, and store releases alter the aircraft's dynamics. Additionally, icing and damage from ice accumulation or battle damage can dramatically alter aerodynamics in unpredictable ways.
...
Industrial robots face continuous uncertainty from multiple sources. Payload variations are particularly significant, as a robot arm might handle payloads from empty gripper to maximum capacity, changing effective inertia by factors of 10 or more. Wear and degradation accumulate over millions of cycles through gearbox backlash, bearing wear, and cable stretching. Friction variations depend on temperature, lubrication, and contamination, affecting both static and dynamic friction characteristics. Furthermore, high-speed robots excite structural resonances that depend on configuration, introducing flexible dynamics into the system.
...
Modern power grids face unprecedented uncertainty from renewable generation. Solar variability poses a significant challenge, as cloud passages can change solar farm output by 50\% in minutes. Wind intermittency causes wind power to fluctuate on time scales from seconds (gusts) to seasons. Demand uncertainty arises because electric vehicle charging and smart appliances create unpredictable load patterns. Grid topology changes further complicate matters, as lines are switched for maintenance, faults, and optimization.
...
Medical control systems face patient-to-patient variability and the ultimate consequences of failure. Drug delivery systems must account for patient metabolism that varies by factors of 3--10, where a dose appropriate for one patient may be dangerous for another. Artificial organs present unique challenges because blood pump controllers must accommodate varying vascular resistance, patient activity, and device degradation over time. Radiation therapy systems must adapt as tumor size and location change during treatment, requiring beam control to continuously adjust.
...
Good control requires careful frequency-domain shaping of these functions. At low frequencies, the sensitivity magnitude $|S|$ should be small for effective tracking and disturbance rejection, which means we want $|S(j\omega)| \ll 1$ and thus require $|L(j\omega)| \gg 1$. At high frequencies, the complementary sensitivity magnitude $|T|$ should be small for noise rejection and robustness to unmodeled dynamics, which means we want $|T(j\omega)| \ll 1$ and thus require $|L(j\omega)| \ll 1$.
...
Recommended design specifications for robust systems depend on the application criticality. For good robustness in typical applications, $M_S \leq 2$ (6 dB) is recommended. For critical applications where reliability is paramount, $M_S \leq 1.5$ (3.5 dB) provides additional safety margin. For robustness to multiplicative uncertainty, $M_T \leq 1.25$ (2 dB) ensures adequate high-frequency roll-off.
...
This formulation simultaneously shapes both sensitivity functions. The weight $W_S$ is designed to be large at low frequencies, forcing $|S|$ to be small in that range and ensuring good tracking performance. Conversely, $W_T$ is large at high frequencies, forcing $|T|$ small there and providing robustness to unmodeled dynamics.
...
For SISO systems, classical margins remain valuable. A gain margin $G_m \geq 6$ dB (factor of 2) provides adequate tolerance to gain variations. A phase margin $\phi_m \geq 45°$ ensures sufficient tolerance to phase lag from unmodeled dynamics or time delays. The delay margin $\tau_m = \phi_m/\omega_c$ should be at least one sampling period for digital systems to maintain stability under discretization effects.

\subsubsection{Sensitivity Peak Limits}

Specify bounds on $M_S$ and $M_T$ to ensure adequate robustness. Typically, $M_S \leq 2$ (6 dB) provides good disturbance rejection with acceptable stability margins, while $M_T \leq 1.25$ (2 dB) ensures robustness to multiplicative uncertainty at high frequencies.
...
From the step response, measure the steady-state velocity $\omega_{ss} = K_{\text{DC}} \cdot V_{in}$ to determine the DC gain, and measure the time constant $\tau$ as the time to reach 63\% of steady-state.

\begin{algorithm}
\caption{System Identification via Step Response}
\label{alg:sysid}
\begin{algorithmic}[1]
\State \textbf{Initialize:} Configure motor PWM output and encoder inputs
\State \textbf{Initialize:} Set up encoder interrupt for position counting
\State \textbf{Initialize:} Create arrays for velocity samples and timestamps
\State Wait 2 seconds for system to stabilize
\State Record start time $t_{\text{start}}$
\State Apply step input to motor (constant PWM duty cycle)
\While{sample count $<$ 500 AND elapsed time $<$ 5 seconds}
    \If{10 ms elapsed since last sample}
        \State Read current encoder count
        \State Compute velocity: $\omega = \Delta\theta / \Delta t$
        \State Store velocity and timestamp in arrays
        \State Increment sample index
    \EndIf
\EndWhile
\State Set motor PWM to zero (stop motor)
\State Output all time-velocity data pairs for analysis
\State \textbf{Post-processing:} Fit first-order model $G(s) = K_{\text{DC}}/(\tau s + 1)$
\State Extract $K_{\text{DC}}$ from steady-state velocity and $\tau$ from 63\% rise time
\State \Return Model parameters $K_{\text{DC}}$, $\tau$
\end{algorithmic}
\end{algorithm}
...
The design criteria for this aggressive controller are a bandwidth $\omega_c = 2/\tau^{(0)}$ (twice the nominal pole frequency) and a phase margin $\phi_m \approx 30°$ (intentionally low for demonstration purposes to show how marginal stability leads to problems under perturbation).

\subsubsection{Robust Controller (Conservative)}

Design a PI controller with conservative margins, using a bandwidth $\omega_c = 0.5/\tau^{(100g)}$ (based on the slowest plant with maximum payload) and ensuring a phase margin $\phi_m \geq 60°$ to provide adequate robustness across all payload conditions.

\begin{algorithm}
\caption{PI Controller with Anti-Windup}
\label{alg:pi_control}
\begin{algorithmic}[1]
\State \textbf{Parameters:} $K_p$ (proportional gain), $K_i$ (integral gain), $I_{\max}$ (integral limit)
\State \textbf{Initialize:} $I \gets 0$ (integral accumulator)
\State
\Procedure{ComputeControl}{$e$, $\Delta t$}
    \State $I \gets I + e \cdot \Delta t$ \Comment{Integrate error}
    \State $I \gets \text{clamp}(I, -I_{\max}, I_{\max})$ \Comment{Anti-windup saturation}
    \State $u \gets K_p \cdot e + K_i \cdot I$ \Comment{PI control law}
    \State \Return $u$
\EndProcedure
\State
\State \textbf{Controller Configurations:}
\State \quad Aggressive: $K_p = 5.0$, $K_i = 20.0$, $I_{\max} = 100$
\State \quad Robust: $K_p = 1.5$, $K_i = 3.0$, $I_{\max} = 100$
\State
\State \textbf{Main Control Loop:}
\While{system running}
    \State Compute time step $\Delta t$ since last iteration
    \State Read setpoint $r$ from input device
    \State Read position $y$ from encoder
    \State Compute error: $e \gets r - y$
    \State Compute control: $u \gets \text{ComputeControl}(e, \Delta t)$
    \State Apply saturated output: $u_{\text{sat}} \gets \text{clamp}(u, -u_{\max}, u_{\max})$
    \State Send $u_{\text{sat}}$ to motor driver
    \State Log data at 20 Hz for analysis
\EndWhile
\end{algorithmic}
\end{algorithm}
...
\begin{examplebox}[Calculating Stability Margins from Bode Plot]
...
\textbf{Assessment:} The gain margin of 7.6 dB is adequate for most applications. However, the phase margin of 4° is \textit{unacceptable} and indicates the system is dangerously close to instability. The system will likely oscillate with any parameter variation. The recommendation is to add phase lead compensation to increase the phase margin to at least 45°.

$\boxed{G_m = 7.6 \text{ dB}, \quad \phi_m = 4°}$
\end{examplebox}

\begin{examplebox}[Robust Stability via Small Gain Theorem]
...
Taking the derivative and setting to zero, or evaluating at several frequencies, we find: at $\omega = 0$, $|W_m T| = 0$; at $\omega = 10$, $|W_m T| = \frac{50}{\sqrt{200} \cdot \sqrt{221}} = \frac{50}{14.1 \cdot 14.9} = 0.24$; at $\omega = 100$, $|W_m T| = \frac{500}{\sqrt{10100} \cdot \sqrt{10221}} = \frac{500}{100.5 \cdot 101.1} = 0.049$; and as $\omega \to \infty$, $|W_m T| \to 0$.
...
Design a lead compensator for the plant $G(s) = \dfrac{10}{s(s+5)}$ to achieve a phase margin $\geq 50°$ with gain crossover frequency $\omega_c = 5$ rad/s.
...
Maximum deviation occurs at the extremes of the uncertainty range: for $J = 0.008$, the relative error is $(0.01 - 0.008)/0.008 = +25\%$, while for $J = 0.015$, the relative error is $(0.01 - 0.015)/0.015 = -33\%$.
...
A feedback system has nominal plant $G_{\text{nom}}(s) = \dfrac{1}{s+1}$, additive uncertainty bounded by $|\Delta_a(j\omega)| \leq \dfrac{0.5\omega}{\omega + 5}$, and controller $K(s) = 2$. Determine if the system is robustly stable.
...
\begin{exercise}
For a system with loop transfer function $L(s) = \dfrac{50}{s(s+1)(s+10)}$, calculate the gain margin and phase margin. Assess whether the system meets typical robustness specifications.
\end{exercise}

\begin{exercise}
A control system has sensitivity function $S(s) = \dfrac{s^2 + 2s + 1}{s^2 + 3s + 10}$. Calculate $M_S = \|S\|_\infty$ and determine the guaranteed minimum gain and phase margins.
\end{exercise}

\begin{exercise}
A plant has multiplicative uncertainty weight $W_m(s) = \dfrac{s+1}{s+20}$. If the complementary sensitivity at $\omega = 10$ rad/s has magnitude $|T(j10)| = 0.5$, is the robust stability condition satisfied at this frequency?
\end{exercise}

\begin{exercise}
A feedback system has $G(s) = \dfrac{2}{(s+1)^2}$ and $K(s) = 3$. An additive uncertainty $\Delta_a$ satisfies $|\Delta_a(j\omega)| \leq 0.1\omega^2$ for all $\omega$. Determine if the system is robustly stable.
\end{exercise}

\begin{exercise}
Design a lead compensator for $G(s) = \dfrac{5}{s(s+2)}$ to achieve phase margin $\geq 60°$ with gain crossover frequency $\omega_c = 4$ rad/s.
\end{exercise}

\begin{exercise}
A spring-mass system has uncertain stiffness $k \in [80, 120]$ N/m with nominal $k_0 = 100$ N/m and mass $m = 1$ kg. Express the plant uncertainty in multiplicative form and determine the weight $W_m(s)$.
\end{exercise}

\begin{exercise}
Explain the physical meaning of minimizing $\|W_S S\|_\infty$ where $W_S(s) = \dfrac{s/M + \omega_B}{s + \omega_B A}$ with $M = 2$, $\omega_B = 10$ rad/s, and $A = 0.01$.
\end{exercise}

\begin{exercise}
In the robust motor control experiment, a student finds that the aggressive controller becomes unstable when payload mass increases from 0g to 50g. If the nominal time constant is $\tau_0 = 0.1$ s and increases to $\tau = 0.15$ s with the 50g payload, calculate how much the phase margin at the original crossover frequency has decreased.
\end{exercise}

\begin{exercise}
Describe a procedure to assess robust performance using Monte Carlo simulation for a system with three uncertain parameters, each uniformly distributed over a $\pm 20\%$ range.
\end{exercise}

\begin{exercise}
A system requires $|S(j\omega)| < 0.1$ for $\omega < 1$ rad/s and $|T(j\omega)| < 0.1$ for $\omega > 100$ rad/s. Sketch a typical sensitivity/complementary sensitivity pair that meets these requirements and identify the crossover frequency range.
\end{exercise}
