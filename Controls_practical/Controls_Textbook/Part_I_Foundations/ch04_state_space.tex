...
\begin{itemize}[noitemsep]
    \item Understand the historical development and motivation for state-space methods
    \item Define state variables and formulate state-space models
    \item Convert between transfer function and state-space representations
    \item Analyze system dynamics using eigenvalues and the state transition matrix
    \item Apply state-space modeling to practical multi-body systems
\end{itemize}
...
\begin{enumerate}
    \item \textbf{MIMO Complexity:} For systems with multiple inputs and outputs, transfer functions become unwieldy matrices of rational polynomials, losing the intuitive interpretation that made them valuable.

    \item \textbf{Initial Conditions:} Transfer functions assume zero initial conditions, making them ill-suited for trajectory planning or systems that must operate from arbitrary starting points.

    \item \textbf{Internal Behavior:} Transfer functions describe only input-output relationships, potentially hiding unstable internal dynamics---a phenomenon later formalized as ``observability'' and ``controllability.''

    \item \textbf{Time-Varying Systems:} Classical methods assume linear time-invariant (LTI) systems; extending them to time-varying systems required fundamental reformulation.
\end{enumerate}
...
\begin{itemize}
    \item \textbf{Multiple Coupled Variables:} A spacecraft's position and orientation in three-dimensional space require tracking at least twelve state variables (position, velocity, attitude, and angular rates).

    \item \textbf{Sensor Fusion:} Navigation required combining measurements from multiple imperfect sensors---gyroscopes, accelerometers, star trackers, and radar---each with different noise characteristics and update rates.

    \item \textbf{Fuel Optimality:} With strictly limited fuel, controllers had to minimize thrust usage while achieving precise trajectory corrections.

    \item \textbf{Real-Time Computation:} All calculations had to execute on primitive onboard computers with kilobytes of memory and kilohertz clock speeds.
\end{itemize}
...
\begin{itemize}
    \item \textbf{Computed Torque Control:} Linearization of nonlinear dynamics about the current state, enabling linear state feedback.
    \item \textbf{State Estimation:} Kalman filters combine joint encoder measurements with dynamic models to estimate unmeasured states like joint velocities.
    \item \textbf{Trajectory Optimization:} Pontryagin's maximum principle determines time-optimal or energy-optimal joint trajectories.
\end{itemize}
...
A simple pendulum's motion is governed by $\ddot{\theta} + (g/L)\sin\theta = 0$. To predict future motion, we need:
\begin{itemize}
    \item $x_1 = \theta$ (angular position)
    \item $x_2 = \dot{\theta}$ (angular velocity)
\end{itemize}
Knowing only $\theta$ is insufficient---we cannot distinguish a pendulum at rest from one passing through the same angle with high velocity.
\end{example}
...
where:
\begin{itemize}
    \item $\state(t) \in \Real^n$ is the state vector
    \item $\inputvec(t) \in \Real^m$ is the input vector
    \item $\outputvec(t) \in \Real^p$ is the output vector
    \item $\mat{A} \in \Real^{n \times n}$ is the \textbf{system matrix} (or state matrix)
    \item $\mat{B} \in \Real^{n \times m}$ is the \textbf{input matrix}
    \item $\mat{C} \in \Real^{p \times n}$ is the \textbf{output matrix}
    \item $\mat{D} \in \Real^{p \times m}$ is the \textbf{feedthrough matrix} (often zero)
\end{itemize}
...
\begin{enumerate}
    \item Identify the highest derivative of the output in the differential equation.
    \item Choose state variables as the output and its successive derivatives.
    \item Write each derivative as a function of states and inputs.
    \item Assemble into matrix form.
\end{enumerate}
...
\begin{itemize}
    \item \textbf{Stable system} (negative real part)
    \item \textbf{Oscillatory response} (nonzero imaginary part)
    \item \textbf{Damped oscillation frequency:} $\omega_d = 1.39$ rad/s
    \item \textbf{Time constant:} $\tau = 1/0.25 = 4$ s
\end{itemize}
\end{example}
...
\begin{enumerate}
    \item $e^{\mat{A} \cdot 0} = \mat{I}$
    \item $\frac{\diff}{\diff t}e^{\mat{A}t} = \mat{A}e^{\mat{A}t} = e^{\mat{A}t}\mat{A}$
    \item $e^{\mat{A}(t_1+t_2)} = e^{\mat{A}t_1}e^{\mat{A}t_2}$
    \item $(e^{\mat{A}t})^{-1} = e^{-\mat{A}t}$
\end{enumerate}
...
\begin{enumerate}
    \item \textbf{Laplace Transform:} $e^{\mat{A}t} = \mathcal{L}^{-1}\{(s\mat{I}-\mat{A})^{-1}\}$
    \item \textbf{Eigenvalue Decomposition:} If $\mat{A} = \mat{V}\mat{\Lambda}\mat{V}^{-1}$, then $e^{\mat{A}t} = \mat{V}e^{\mat{\Lambda}t}\mat{V}^{-1}$
    \item \textbf{Cayley-Hamilton:} For $n \times n$ matrix, $e^{\mat{A}t}$ is a polynomial of degree $n-1$ in $\mat{A}$
\end{enumerate}
...
\begin{enumerate}
    \item Build a physical two-mass system
    \item Derive and validate a state-space model
    \item Observe the complexity this system would have in transfer function form
    \item Compare model predictions with measured responses
\end{enumerate}
...
\begin{itemize}
    \item Two carts (3D printed or commercial dynamics carts, $m_1 \approx m_2 \approx 0.5$ kg)
    \item Linear track (drawer slides or commercial air track, length $\geq$ 1 m)
    \item Two springs ($k_1, k_2 \approx 10$--50 N/m)
    \item Rotary encoders or ultrasonic distance sensors for position measurement
    \item Solenoid or motor for applying controlled force $F(t)$
    \item Data acquisition system (Arduino, myRIO, or similar)
    \item Computer with MATLAB/Python for analysis
\end{itemize}

\textbf{3D Printing Notes:} Cart designs should include:
\begin{itemize}
    \item Low-friction wheel mounts compatible with track
    \item Spring attachment points
    \item Encoder wheel mount or reflector for distance sensor
    \item Estimated print time: 2--3 hours per cart at 0.2 mm layer height
\end{itemize}
...
\begin{enumerate}
    ...
    \begin{itemize}
        \item Weigh carts to determine $m_1$, $m_2$
        \item Measure spring constants by hanging known masses
        \item Estimate damping from free oscillation decay
    \end{itemize}

    \item \textbf{Model Validation:}
    \begin{itemize}
        \item Displace cart 2 and release (initial condition response)
        \item Record $x_1(t)$ and $x_2(t)$
        \item Simulate state-space model with same initial conditions
        \item Compare simulated and measured trajectories
    \end{itemize}

    \item \textbf{Forced Response:}
    \begin{itemize}
        \item Apply step force to cart 2
        \item Apply sinusoidal force at various frequencies
        \item Identify resonant frequencies (should match eigenvalue imaginary parts)
    \end{itemize}

    \item \textbf{Phase Portrait Analysis:}
    \begin{itemize}
        \item Plot $x_1$ vs $\dot{x}_1$ (cart 1 phase plane)
        \item Plot $x_2$ vs $\dot{x}_2$ (cart 2 phase plane)
        \item Observe spiral trajectories indicating damped oscillation
    \end{itemize}
\end{enumerate}
