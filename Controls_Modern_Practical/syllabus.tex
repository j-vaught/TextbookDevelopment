...
All projects must be completed in teams of \textbf{2--3 students}. \textbf{Single-person projects are not permitted} due to insufficient time for individual presentations and the collaborative nature of real-world engineering practice.

Students may self-organize into teams during the first lecture. After the first lecture, any remaining unassigned students will be placed into teams by the instructor. \textbf{No team changes are permitted until after Project~1 is complete.} Requests for changes after Project~1 must be submitted in writing with justification and are subject to instructor approval.
...
\begin{center}
\textbf{Weight: 15\% of Final Grade}
\end{center}
...
\begin{center}
\textbf{Weight: 25\% of Final Grade}
\end{center}
...
Systems should have \textbf{multiple inputs and outputs} (preferably many I/O channels). \textbf{Implementation must be discrete-time.} Each project \textbf{must include both} an ML-based controller (e.g., reinforcement learning, neural network control, adaptive methods) \textbf{and} a classical controller for comparison (e.g., LQR, PID, state feedback). Projects are \textbf{open-ended}---students are encouraged to continue/expand their Project~1 system or propose a new system.
...
All projects require demonstration of the implemented control system. A recorded \textbf{video demonstration} is \textbf{required} and must be uploaded with the project submission. Live demonstrations during presentations are permitted \textit{only} if the equipment is small and portable; large or fragile setups should be demonstrated via video only.
...
Both projects require the following deliverables. \textbf{All items are mandatory unless explicitly stated otherwise.}
...
\addlinespace
Comparison Rigor (Project~2) & Depth and fairness of comparison between classical and ML-based controllers. Both approaches must be given equal effort and evaluation. \\
\bottomrule
\end{tabular}
\end{table}
...
\centering
\textbf{Important:} Project proposals must be approved before work begins.\\
...
\midrule
Application Assignments & AI tools \textbf{allowed}. Must explain approach, interpret results, and verify correctness. \\
Theory Assignments & Submissions must reflect the \textbf{student's own reasoning}. \\
Exams & \textbf{No AI tools permitted.} \\
...
\noindent In the schedule table, \textbf{Exam} rows indicate exam sessions, \textbf{Presentation} rows indicate project presentation sessions, and \textbf{bold text} indicates homework due dates and project milestones.

\vspace{0.5em}

\noindent\textbf{HW0 (Math Prerequisites):} Due \textbf{Sunday, August 16} at midnight.
